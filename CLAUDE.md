...
- **رقم نسخة منفصل تمامًا عن APP_VERSION:** `src/App.jsx` → `const LATEST_APK_VERSION = "V0.0X.YY";` (قريب من APP_VERSION في نفس المكان تقريبًا). ده بيتحدث **بس** لما APK حقيقي يتبني فعلاً ويتحط في `dist/releases/` — مش في كل push زي APP_VERSION (كتير من الـ pushes بتبقى ويب بس من غير APK جديد). بيتستخدم في زرار "📱 Download Android App" اللي ظاهر لزوار الويب على أندرويد بس (شاشة Settings). لو ده اتنسى ومتحدثش وقت بناء APK حقيقي، الزرار هيرشّح لينك لملف مش موجود فعليًا على الـ Hosting.
