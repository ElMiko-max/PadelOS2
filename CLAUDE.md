...
7. **تسليم الـ APK فعليًا:**
   ...
   - **الترتيب الصحيح دايمًا:** (1) `npm run build` (2) `npx cap sync` (3) `gradlew assembleProdDebug` (مش `assembleDebug` لوحدها — بعد الـ product flavors بقى فيه prod/dev، والأمر ده هو اللي بيبني نسخة الإنتاج بس؛ الـ APK بيطلع في `android/app/build/outputs/apk/prod/debug/app-prod-debug.apk`) — في اللحظة دي `dist/` و`android/assets` نضيفين تمامًا من أي APK — **(4) بعد كده بس** انسخ الـ APK الجديد لـ `dist/releases/Matchkeeper-V0.0X.YY-debug.apk` مباشرة (من غير ما يعدي على `public/` خالص) **(4.5)** اكتب/حدّث `dist/releases/latest.json` بمحتوى `{"version":"V0.0X.YY"}` بنفس رقم النسخة بالظبط — ده اللي بيخلي زرار "Android App" في التطبيق يعرض الرقم الصح أوتوماتيك من غير تعديل يدوي تاني في `App.jsx` **(5)** `firebase deploy --only hosting` فورًا من غير ما تعمل `npm run build` تاني بعد النسخ (لإن ده هيمسح الـ APK والـ latest.json اللي حطيتهم في `dist/`).
