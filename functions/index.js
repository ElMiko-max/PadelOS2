...
// entries / 1,048,349 bytes, right at Firestore's hard 1 MiB per-document limit, so every
// further write from the client was silently rejected — this trigger simply stopped firing at
// all, with no error anywhere the admin would see. Fixed at the source: notifications now live
// one-document-per-notification in padelos_notifications/{id} (same shape padelos_audit already
// uses successfully), so this is a plain onDocumentCreated — no diffing needed, since a new
// document IS a new notification, and no single document can ever hit a size ceiling again
// regardless of how many notifications the app has sent in total.
exports.sendPushOnNotification = onDocumentCreated("padelos_notifications/{notifId}", async (event) => {
  const notif = event.data.data();
  console.log(`[push] trigger fired. notifId=${event.params.notifId} userId=${notif.userId} title="${notif.title}"`);
  if (!notif.userId) {
