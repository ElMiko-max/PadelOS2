...
// Tap "Open Location" → small picker so the person chooses which app to open it in,
// instead of always forcing Google Maps. Waze/Apple Maps need coordinates; if the venue's
// link has none, only the Google Maps option (using the original link) is offered.
function MapOpenPicker({mapsUrl,label="📍 Open Location"}){
  ...
  const coords = parseLatLngFromUrl(mapsUrl);
  const apps = [
    {label:"Google Maps", icon:"🗺️", url: coords?`https://www.google.com/maps/search/?api=1&query=${coords.lat},${coords.lng}`:mapsUrl},
    ...(coords?[
      {label:"Waze", icon:"🚗", url:`https://waze.com/ul?ll=${coords.lat},${coords.lng}&navigate=yes`},
      {label:"Apple Maps", icon:"🍏", url:`https://maps.apple.com/?ll=${coords.lat},${coords.lng}`},
    ]:[]),
  ];
  return <div ref={ref} style={{position:"relative",display:"inline-block"}}>
    <SmBtn label={label} onClick={e=>{e.stopPropagation();setOpen(o=>!o);}} color="#6366F1"/>
    {open&&<div style={{position:"absolute",top:32,left:0,zIndex:50,background:"var(--po-card)",border:"0.5px solid var(--po-bdr)",borderRadius:10,minWidth:170,boxShadow:"0 8px 24px #00000055",overflow:"hidden"}}>
      {apps.map(a=><a key={a.label} href={a.url} target="_blank" rel="noopener noreferrer" onClick={()=>setOpen(false)}
        style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",fontSize:13,color:"var(--po-text)",textDecoration:"none",borderBottom:"0.5px solid var(--po-bdr)"}}>
...
function VenueMapCard({venue}){
  ...
  if (!venue?.mapsUrl) return null;
  const coords = parseLatLngFromUrl(venue.mapsUrl);
  const checkDistance = () => {
  ...
  return <Card style={{marginBottom:10}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
      <MapOpenPicker mapsUrl={venue.mapsUrl}/>
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive,onViewProfile,onSwapCTBreak,onRegenCTBreaks,onToggleExempt,onTogglePaid,onUpdateEventFinance,onSetMatchModeStart}){
  ...
  return <>
    ...
    {tab==="info"&&<VenueMapCard venue={venue}/>}
