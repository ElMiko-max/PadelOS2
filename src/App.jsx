...
function buildFullBreakTableCard(ev, venue, plan, tc, communityName){
  ...
  const nameW = 150, colW = 46;
  const w = Math.max(CARD_W, nameW + plan.rounds.length*colW + 48);
  const h = 140 + 60 + 36 + players.length*32 + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  drawHeader(ctx, w, "Break Schedule", `${plan.rounds.length} rounds · ${tc} courts`, communityName);
  ...
  plan.rounds.forEach((r,ri)=>ctx.fillText(`R${ri+1}`, 28+nameW+ri*colW+14, y));
  y += 14;
  ...
  players.forEach((p,i)=>{
    ...
    roundRect(ctx, 24, y-18, w-48, 28, 8); ctx.fill();
    ctx.fillStyle = COLORS.text; ctx.font="600 12px Arial";
    ctx.fillText(p.nickname, 32, y);
    plan.rounds.forEach((r,ri)=>{
      const onBreak = (r.onBreak||[]).some(b=>b.userId===p.userId);
      ctx.fillStyle = onBreak ? COLORS.amber : "#D1D5DB";
      ctx.font = onBreak ? "700 13px Arial" : "12px Arial";
      ctx.textAlign = "center";
      ctx.fillText(onBreak ? "☕" : "·", 28+nameW+ri*colW+18, y+2);
      ctx.textAlign = "left";
    });
    y += 32;
  });
...
function buildRound1Card(ev,venue,plan,tc,communityName){
  ...
  const h = 140 + 60 + 50 + r1.matches.length*78 + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  drawHeader(ctx, w, "Round 1 Matches", `${plan.totalRounds} rounds total · ${tc} courts`, communityName);

  let y = 168;
  y = drawEventStrip(ctx, w, y, ev, venue);

  if((r1.onBreak||[]).length>0){
    ctx.fillStyle = COLORS.card; roundRect(ctx, 24, y, w-48, 50, 14); ctx.fill();
    ctx.strokeStyle = COLORS.border; roundRect(ctx, 24, y, w-48, 50, 14); ctx.stroke();
    ctx.fillStyle = COLORS.amber; ctx.font="700 13px Arial";
    ctx.fillText("☕ On break this round:", 40, y+20);
    ctx.fillStyle = COLORS.text; ctx.font="600 14px Arial";
    ctx.fillText(r1.onBreak.map(p=>p.nickname).join(", "), 40, y+38);
    y += 66;
  }

  r1.matches.forEach(m=>{
    ctx.fillStyle = COLORS.card; roundRect(ctx, 24, y, w-48, 66, 14); ctx.fill();
    ctx.strokeStyle = COLORS.border; roundRect(ctx, 24, y, w-48, 66, 14); ctx.stroke();
    ctx.fillStyle = COLORS.accent; ctx.font="700 13px Arial";
    ctx.fillText(`COURT ${m.court}`, 40, y+24);
    ctx.fillStyle = COLORS.text; ctx.font="600 16px Arial";
    const teamA = m.teamA.map(p=>p.nickname).join(" & ");
    const teamB = m.teamB.map(p=>p.nickname).join(" & ");
    ctx.fillText(teamA, 40, y+48);
    ctx.fillStyle = COLORS.dim; ctx.font="600 13px Arial";
    ctx.fillText("vs", 40 + ctx.measureText(teamA).width + 10, y+48);
    ctx.fillStyle = COLORS.text; ctx.font="600 16px Arial";
    ctx.fillText(teamB, 40 + ctx.measureText(teamA).width + 36, y+48);
    y += 78;
  });
...
function drawEventStrip(ctx, w, y, ev, venue){
  ...
  ctx.fillStyle = COLORS.card; roundRect(ctx, 24, y, w-48, 38, 10); ctx.fill();
  ctx.strokeStyle = COLORS.border; roundRect(ctx, 24, y, w-48, 38, 10); ctx.stroke();
  ctx.fillStyle = COLORS.text; ctx.font="700 14px Arial";
  ctx.fillText(ev.name, 36, y+16);
  ctx.fillStyle = COLORS.sub; ctx.font="12px Arial";
  ctx.fillText(text, 36, y+32);
  return y + 50;
}

function buildStandingsCard(ev,venue,ciStands,tc,maxPts,communityName){
  const w = CARD_W;
  const h = 140 + 60 + 60 + ciStands.length*74 + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  ...
  ciStands.forEach((s,i)=>{
    ...
    roundRect(ctx, 24, y, w-48, 64, 14); ctx.fill();
    ctx.strokeStyle = isTop ? "#FDE68A" : COLORS.border;
    roundRect(ctx, 24, y, w-48, 64, 14); ctx.stroke();

    ctx.fillStyle = isTop ? COLORS.amber : i===1?"#64748B":i===2?"#B45309":COLORS.text;
    ctx.font = "700 19px Arial";
    ctx.fillText(`${isTop?"🏆":i+1+"."} ${s.user.nickname}`, 40, y+28);
    ctx.fillStyle = COLORS.dim; ctx.font="14px Arial";
    ctx.fillText(`${s.wins}W · ${s.breaks} breaks · PES ${pes}%`, 40, y+50);

    ctx.fillStyle = COLORS.accent; ctx.font="700 26px Arial"; ctx.textAlign="right";
    ctx.fillText(`${s.pts}`, w-40, y+36);
    ctx.fillStyle = COLORS.dim; ctx.font="12px Arial";
    ctx.fillText("pts", w-40, y+53); ctx.textAlign="left";
    y += 74;
  });
...
function buildResultsTableCard(ev,venue,plan,ciStands,tc,maxPts,communityName){
  const colW=64, nameW=160, pesW=56, totalW=56;
  const w = Math.max(CARD_W, nameW+plan.rounds.length*colW+pesW+totalW+48);
  const h = 140 + 60 + 50 + ciStands.length*36 + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
...
function buildRoundResultsCard(ev,venue,plan,communityName){
  const w = CARD_W;
  let estH = 140 + 60;
  plan.rounds.forEach(r=>{ estH += 34 + r.matches.length*64; });
  const h = estH + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  ...
  y = drawEventStrip(ctx, w, y, ev, venue);
  plan.rounds.forEach((r,ri)=>{
    ctx.fillStyle = COLORS.text; ctx.font="700 18px Arial";
    ctx.fillText(`Round ${ri+1}`, 24, y); y += 28;
    r.matches.forEach(m=>{
      const teamA = m.teamA.map(p=>p.nickname).join(" & ");
      const teamB = m.teamB.map(p=>p.nickname).join(" & ");
      const winT = m.winner==="A"?teamA:m.winner==="B"?teamB:null;
      ctx.fillStyle = COLORS.card; roundRect(ctx, 24, y-18, w-48, 54, 12); ctx.fill();
      ctx.strokeStyle = COLORS.border; roundRect(ctx, 24, y-18, w-48, 54, 12); ctx.stroke();
      ctx.fillStyle = COLORS.dim; ctx.font="700 12px Arial";
      ctx.fillText(`COURT ${m.court}`, 40, y-2);
      ctx.fillStyle = COLORS.text; ctx.font="600 15px Arial";
      ctx.fillText(`${teamA}  vs  ${teamB}`, 40, y+20);
      if(winT){ ctx.fillStyle = COLORS.green; ctx.font="700 13px Arial"; ctx.textAlign="right"; ctx.fillText(`✓ ${winT}`, w-40, y+6); ctx.textAlign="left"; }
      y += 64;
    });
    y += 10;
  });
...
function BBtn({onBack,label="Back",sticky=false,subLabel,eventLabel}){
  ...
  return <div style={{position:"sticky",top:72,zIndex:40,background:"var(--po-bg)",marginLeft:-12,marginRight:-12,paddingLeft:12,paddingRight:12,marginBottom:8,borderBottom:"0.5px solid var(--po-bdr)"}}>{content}</div>;
}
...
function TopBar({me,nav,menu,setMenu,onNav,onProfile,onVenues,onSettings,TH,dark}){
  ...
  return <div style={{background:TH?.nav||"#0E1117",borderBottom:`0.5px solid ${TH?.border||"var(--po-bdr)"}`,padding:"0 12px",display:"flex",alignItems:"center",justifyContent:"space-between",height:72,position:"sticky",top:0,left:0,right:0,width:"100%",zIndex:50,transition:"all 0.2s",boxSizing:"border-box"}}>
    <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
      ...
      <div style={{display:"flex",flexDirection:"column",lineHeight:1.1}}>
        <span style={{fontSize:13,fontWeight:600,color:dark?"#F1F5F9":"#FFFFFF"}}>PadelOS</span>
        <span style={{fontSize:9,fontWeight:400,color:dark?"#F1F5F9":"#FFFFFF",opacity:0.6}}>v{APP_VERSION}</span>
      </div>
    </div>
    <div style={{display:"flex",gap:10,flex:1,justifyContent:"center"}}>{tabs.map(t=>{
      const active = nav===t.k;
      return <button key={t.k} onClick={()=>onNav(t.k)} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 20px 9px 9px",borderRadius:14,border:"none",fontSize:15,fontWeight:800,cursor:"pointer",minHeight:50,background:active?"rgba(255,255,255,0.97)":"rgba(255,255,255,0.16)",transition:"all 0.15s"}}>
        <div style={{width:38,height:38,borderRadius:12,background:t.chip,color:t.iconColor,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transform:`rotate(${t.rot}deg)`}}>{t.icon}</div>
        <span style={{color:active?"#4F46E5":"rgba(255,255,255,0.92)"}}>{t.l}</span>
      </button>;
    })}</div>
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive}){
  ...
  async function handleShareAfter(){
    ...
    try{
      ...
      const cards=[buildStandingsCard(effEv,venue,ciStands,tc,mp,comm.name)];
      if(plan) cards.push(buildResultsTableCard(effEv,venue,plan,ciStands,tc,mp,comm.name));
