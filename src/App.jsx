...
function CTMatchesTab({plan,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,totalBookingMin,eventTime,onSetMatchModeStart}){
  ...
  return <>
    ...
    {reversedRounds.map((round,revIdx)=>{
      ...
      return <div key={ri} style={{marginBottom:20,opacity:isLatest?1:0.7}}>
        ...
        {isLatest&&<MatchTimerWidget plan={plan} roundNum={round.roundNum} roundDuration={plan.roundDuration} totalRounds={plan.maxRounds} totalBookingMin={totalBookingMin} eventTime={eventTime} onStart={onSetMatchModeStart}/>}
        {isLeague
...
// ══════════════════════════════════════════════════════
function MatchTimerWidget({plan,roundNum,roundDuration,totalRounds,totalBookingMin,eventTime,onStart}){
  ...
  const endAt   = started ? new Date(plan.matchModeStartAt).getTime() + (offsets[roundNum]||roundNum*rd)*60000 : null;
  ...
  const isCompressed = started && roundNum>1 && Math.round(restDur)<rd;
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive,onViewProfile,onSwapCTBreak,onRegenCTBreaks,onToggleExempt,onTogglePaid,onUpdateEventFinance,onSetMatchModeStart}){
  ...
  return <>
      ...
      {plan&&<>
        ...
        {[...plan.rounds].reverse().map((round,revIdx)=>{
          ...
          return <div key={ri} style={{marginBottom:24,opacity:isLatest?1:0.75}}>
            ...
            {isLatest&&!isCompleted&&<MatchTimerWidget plan={plan} roundNum={round.round} roundDuration={plan.roundDuration||roundDur} totalRounds={plan.totalRounds} totalBookingMin={durationHrs*60} eventTime={effEv.time} onStart={act.setMatchModeStart}/>}
    ...
    {/* CT MATCHES */}
    {tab==="matches"&&isCT&&isAdmin&&plan&&<CTMatchesTab plan={plan} onSetWinCT={act.setWinCT} onApplyPromo={act.applyPromo} onNextCTLadder={act.nextCTLadder} onSwapCTLadder={act.swapCTLadder} totalBookingMin={durationHrs*60} eventTime={effEv.time} onSetMatchModeStart={act.setMatchModeStart}/>}
