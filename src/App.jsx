...
function TopBar({me,nav,menu,setMenu,onNav,onProfile,onVenues,onSettings,onPlatformAdmin,TH,dark}){
  const tabs = [
  ...
  return <div style={{background:TH?.nav||"#0E1117",borderBottom:`0.5px solid ${TH?.border||"var(--po-bdr)"}`,padding:"0 8px",display:"flex",alignItems:"center",justifyContent:"space-between",height:60,position:"sticky",top:0,left:0,right:0,width:"100%",zIndex:50,transition:"all 0.2s",boxSizing:"border-box",gap:4}}>
    ...
    <div style={{position:"relative",flexShrink:0}} onClick={e=>e.stopPropagation()}>
      ...
      {menu&&<div style={{position:"absolute",right:0,top:42,background:"var(--po-card)",border:"0.5px solid var(--po-bdr)",borderRadius:10,padding:6,minWidth:190,zIndex:100,boxShadow:"0 8px 32px #00000066"}}>
        ...
        {[{i:"👤",l:"My Profile",fn:onProfile},{i:"🛡",l:"Platform Admin",fn:onPlatformAdmin},{i:"🏟",l:"Venues",fn:onVenues},{i:"⚙️",l:"Settings",fn:onSettings},{i:"🚪",l:"Sign Out",fn:()=>setMenu(false),d:true}].map(x=><button key={x.l} onClick={x.fn} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"10px 10px",minHeight:40,borderRadius:7,border:"none",background:"transparent",color:x.d?"#EF4444":"var(--po-sub)",fontSize:13,cursor:"pointer",textAlign:"left"}}>{x.i} {x.l}</button>)}
      </div>}
    </div>
  </div>;
}
        ...
        {(() => {
          ...
          return <>
            ...
            {past.length>0&&<CollapsibleSection label={`📁 Past Events (${past.length})`} defaultOpen={false}>
              {past.map(ev=><EvCard key={ev.id} ev={ev} onClick={()=>onOpenEv(ev.id)}/>)}
            </CollapsibleSection>}
            {isAdmin&&archived.length>0&&<CollapsibleSection label={`📦 Archived (${archived.length})`} defaultOpen={false}>
              {archived.map(ev=><EvCard key={ev.id} ev={ev} onClick={()=>onOpenEv(ev.id)}/>)}
            </CollapsibleSection>}
          </>;
        })()}
      </>}
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive,onViewProfile,onSwapCTBreak,onRegenCTBreaks}){
  ...
  return <>
    ...
    {tab==="players"&&<>
      ...
      {isAdmin&&!ctR1Locked&&<><div style={{display:"flex",gap:6,marginBottom:10}}><Btn label="+ Add Member" onClick={()=>{setSAM(o=>!o);setSAG(false);}} style={{flex:1}}/>{!sim&&<Btn label="+ Add Guest" onClick={()=>{setSAG(o=>!o);setSAM(false);}} style={{flex:1}}/>}</div>
        ...
        return <Card key={r.userId}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"flex-end",alignItems:"center"}}>
              ...
              {isAdmin&&(!effEv.plan||(isCT&&!ctR1Locked))&&<SmBtn label="✕" onClick={()=>act.removeFromEvent(u.id)} color="#EF4444" style={{padding:"4px 8px",fontSize:11}}/>}
            </div>
          </div>
        </Card>;
      })}
...
function ProfileSc({user,comms,onBack,viewedByAdmin}){
  ...
  {tab==="usr"&&<>
    ...
    :<Card style={{padding:0,overflow:"hidden"}}>
      {/* Current USR header strip */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",background:"#6366F122",borderBottom:"0.5px solid var(--po-bdr)"}}>
        <span style={{fontSize:11,color:"var(--po-dim)",fontWeight:600}}>CURRENT USR</span>
        <span style={{fontSize:20,fontWeight:700,color:"#6366F1"}}>{user.usr}</span>
      </div>
      {/* Column headers */}
      ...
      {usrHist.map((h,i)=>{
        ...
        const histUpToNow = histChron.slice(0, idx);
        const paddedPrev = histUpToNow.length < 5
          ? [...Array(5-histUpToNow.length).fill({pes:seedUsr}), ...histUpToNow]
          : histUpToNow.slice(-5);
        const prevUsr = Math.round(paddedPrev.reduce((s,x)=>s+x.pes,0)/paddedPrev.length);
        const paddedNow = [...histUpToNow,h].length < 5
          ? [...Array(Math.max(0,4-histUpToNow.length)).fill({pes:seedUsr}), ...histUpToNow, h]
        ...
        return <div key={i} style={{display:"grid",gridTemplateColumns:"68px 1fr 44px 64px",gap:2,padding:"10px 12px",borderBottom:i<usrHist.length-1?"0.5px solid var(--po-bdr)":"none",alignItems:"center"}}>
          <div style={{fontSize:10,color:"var(--po-dim)"}}>{fmtD(h.date)}</div>
          <div>
            <div style={{fontSize:10,color:"var(--po-dim)"}}>#{h.eventId}</div>
            <div style={{fontSize:12,fontWeight:600,color:"var(--po-text)"}}>{h.eventName}</div>
          </div>
          <div style={{textAlign:"right",fontSize:13,fontWeight:700,color:"#6366F1"}}>{h.pes}%</div>
          <div style={{textAlign:"right"}}>
...
function PlatformAdminSc({users,comms,venues,onBack,onAddUser,onEditUser,onDeleteUser,onViewProfile}){
  ...
  <Tabs tabs={[["users",`Users (${users.length})`],["archived","Archived Events"]]} active={tab} onChange={setTab}/>
