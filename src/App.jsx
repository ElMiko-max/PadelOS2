...
function drawCardBase(ctx,w,h,title,subtitle){
  ...
  ctx.fillText("🏸 PadelOS",24,32);
  ctx.font="600 20px Arial";ctx.fillText(title,24,64);
  if(subtitle){ctx.font="13px Arial";ctx.fillStyle="#E0E7FF";ctx.fillText(subtitle,24,84);}
}
...
function buildEventInfoCard(ev,venue,players){
  const w=900,h=560;
  const c=document.createElement("canvas");c.width=w;c.height=h;
  const ctx=c.getContext("2d");
  drawCardBase(ctx,w,h,ev.name,`${fmtD(ev.date)} · ${ev.time}${ev.timeTo?" → "+ev.timeTo:""}`);
  let y=130;
  ctx.fillStyle="#A5B4FC";ctx.font="14px Arial";
  ctx.fillText(`🏟 ${venue?venue.name+" · "+venue.area:"Venue TBD"}`,24,y); y+=30;
  ctx.fillText(`🎾 ${ev.courts} courts · 👥 ${players.length} players registered`,24,y); y+=40;
  ctx.fillStyle="#fff";ctx.font="bold 16px Arial";ctx.fillText("Registered Players",24,y); y+=10;
  ctx.strokeStyle="#33335A";ctx.beginPath();ctx.moveTo(24,y+10);ctx.lineTo(w-24,y+10);ctx.stroke();
  y+=34;
  const sorted=[...players].sort((a,b)=>b.usr-a.usr);
  const colW=(w-48)/2;
  sorted.forEach((p,i)=>{
    const col=i%2, row=Math.floor(i/2);
    const x=24+col*colW, yy=y+row*34;
    ctx.fillStyle="#1E1B4B";roundRect(ctx,x,yy-20,colW-10,28,6);ctx.fill();
    ctx.fillStyle="#fff";ctx.font="600 13px Arial";
    ctx.fillText(`${i+1}. ${p.nickname}`,x+10,yy-6);
    ctx.fillStyle="#A5B4FC";ctx.font="12px Arial";
    ctx.textAlign="right";ctx.fillText(`USR ${p.usr}`,x+colW-20,yy-6);ctx.textAlign="left";
  });
  return c;
}

function buildBreaksRound1Card(plan,tc){
  const w=900,h=600;
  const c=document.createElement("canvas");c.width=w;c.height=h;
  const ctx=c.getContext("2d");
  drawCardBase(ctx,w,h,"Break Schedule & Round 1",`${plan.totalRounds} rounds · ${tc} courts`);
  let y=120;
  // Break table summary - who's on break round 1
  const r1=plan.rounds[0];
  ctx.fillStyle="#fff";ctx.font="bold 16px Arial";ctx.fillText("Round 1 — On Break",24,y); y+=26;
  ctx.font="13px Arial";ctx.fillStyle="#FBBF24";
  const breakNames=(r1.onBreak||[]).map(p=>p.nickname).join(", ")||"None";
  ctx.fillText(breakNames,24,y); y+=40;

  ctx.fillStyle="#fff";ctx.font="bold 16px Arial";ctx.fillText("Round 1 — Matches",24,y); y+=10;
  ctx.strokeStyle="#33335A";ctx.beginPath();ctx.moveTo(24,y+10);ctx.lineTo(w-24,y+10);ctx.stroke();
  y+=36;
  r1.matches.forEach(m=>{
    ctx.fillStyle="#1E1B4B";roundRect(ctx,24,y-22,w-48,52,8);ctx.fill();
    ctx.fillStyle="#38BDF8";ctx.font="bold 13px Arial";ctx.fillText(`Court ${m.court}`,36,y-2);
    ctx.fillStyle="#fff";ctx.font="14px Arial";
    const teamA=m.teamA.map(p=>p.nickname).join(" & ");
    const teamB=m.teamB.map(p=>p.nickname).join(" & ");
    ctx.fillText(`${teamA}  vs  ${teamB}`,130,y-2);
    y+=64;
  });
  return c;
}

function buildStandingsCard(ciStands,tc,maxPts){
  const w=900,h=140+ciStands.length*48;
  const c=document.createElement("canvas");c.width=w;c.height=h;
  const ctx=c.getContext("2d");
  drawCardBase(ctx,w,h,"Final Standings",`Max possible: ${maxPts||"—"} pts`);
  let y=130;
  ciStands.forEach((s,i)=>{
    const pes = maxPts>0 ? Math.round((s.pts/maxPts)*100*10)/10 : 0;
    ctx.fillStyle=i===0?"#FBBF2422":i%2===0?"#1E1B4B":"#181640";
    roundRect(ctx,24,y-26,w-48,40,8);ctx.fill();
    ctx.fillStyle=i===0?"#FBBF24":i===1?"#94A3B8":i===2?"#CD7C2F":"#fff";
    ctx.font="bold 15px Arial";
    ctx.fillText(`${i===0?"🏆":i+1+"."} ${s.user.nickname}`,36,y-6);
    ctx.fillStyle="#A5B4FC";ctx.font="12px Arial";
    ctx.fillText(`${s.wins}W · ${s.breaks} breaks · ${s.played} played · PES ${pes}%`,36,y+12);
    ctx.fillStyle="#6366F1";ctx.font="bold 18px Arial";ctx.textAlign="right";
    ctx.fillText(`${s.pts} pts`,w-40,y-2);ctx.textAlign="left";
    y+=48;
  });
  return c;
}

function buildResultsTableCard(plan,ciStands,tc,maxPts){
  const colW=70, nameW=140, pesW=60, totalW=70;
  const w=nameW+plan.rounds.length*colW+pesW+totalW+48;
  const h=140+ciStands.length*32;
  const c=document.createElement("canvas");c.width=w;c.height=h;
  const ctx=c.getContext("2d");
  drawCardBase(ctx,w,h,"Match Results Table",`${plan.rounds.length} rounds · Max ${maxPts||"—"} pts`);
  let y=120;
  // Header row
  ctx.fillStyle="#A5B4FC";ctx.font="bold 11px Arial";
  ctx.fillText("Player",24,y);
  plan.rounds.forEach((r,ri)=>ctx.fillText(`M${ri+1}`,24+nameW+ri*colW+20,y));
  ctx.fillText("PES",24+nameW+plan.rounds.length*colW+15,y);
  ctx.fillText("Total",24+nameW+plan.rounds.length*colW+pesW+10,y);
  y+=20;
  ctx.strokeStyle="#33335A";ctx.beginPath();ctx.moveTo(24,y-8);ctx.lineTo(w-24,y-8);ctx.stroke();
  ...
  ciStands.forEach((s,i)=>{
    const pes = maxPts>0 ? Math.round((s.pts/maxPts)*100*10)/10 : 0;
    ctx.fillStyle=i%2===0?"#1E1B4B":"#181640";
    roundRect(ctx,24,y-2,w-48,28,4);ctx.fill();
    ctx.fillStyle="#fff";ctx.font="600 12px Arial";
    ctx.fillText(`${i+1}. ${s.user.nickname} (${s.user.usr})`,30,y+14);
    plan.rounds.forEach((r,ri)=>{
      const cell=cellFor(s.user.id,r);
      ctx.fillStyle=cell.win?"#34D399":cell.text==="Brk"?"#888":"#ccc";
      ctx.font=cell.win?"bold 11px Arial":"11px Arial";
      ctx.fillText(cell.text,24+nameW+ri*colW+20,y+14);
    });
    ctx.fillStyle="#A5B4FC";ctx.font="600 11px Arial";
    ctx.fillText(`${pes}%`,24+nameW+plan.rounds.length*colW+15,y+14);
    ctx.fillStyle="#6366F1";ctx.font="bold 13px Arial";
    ctx.fillText(`${s.pts}`,24+nameW+plan.rounds.length*colW+pesW+12,y+14);
    y+=32;
  });
  return c;
}

function buildRoundResultsCard(plan){
  const w=900;
  let estH=140;
  plan.rounds.forEach(r=>{estH+=34+r.matches.length*30;});
  const h=Math.min(estH,3000);
  const c=document.createElement("canvas");c.width=w;c.height=h;
  const ctx=c.getContext("2d");
  drawCardBase(ctx,w,h,"Round-by-Round Results",`${plan.rounds.length} rounds played`);
  let y=120;
  plan.rounds.forEach((r,ri)=>{
    ctx.fillStyle="#fff";ctx.font="bold 14px Arial";ctx.fillText(`Round ${ri+1}`,24,y);y+=22;
    r.matches.forEach(m=>{
      const teamA=m.teamA.map(p=>p.nickname).join(" & ");
      const teamB=m.teamB.map(p=>p.nickname).join(" & ");
      const winT=m.winner==="A"?teamA:m.winner==="B"?teamB:null;
      ctx.fillStyle="#A5B4FC";ctx.font="12px Arial";
      ctx.fillText(`Court ${m.court}: ${teamA} vs ${teamB}`,36,y);
      if(winT){ctx.fillStyle="#34D399";ctx.font="600 11px Arial";ctx.fillText(`✓ ${winT} won`,36,y+15);}
      y+=30;
    });
    y+=8;
  });
...
export default function PadelOS() {
  ...
  const repairDuplicateIds = () => {
    ...
    setComms(newComms);
    if (fixed > 0) toast2(`Repaired ${fixed} duplicate event ID(s) ✓`);
    else toast2("No duplicate IDs found — data is clean ✓");
  };
  const factoryReset = () => {
    try {
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive}){
  ...
  async function handleShareBefore(){
    ...
    try{
      const cards=[buildEventInfoCard(ev,venue,sharePlayers)];
      if(isCI&&plan&&plan.rounds[0]) cards.push(buildBreaksRound1Card(plan,tc));
      const result = await shareImages(cards, ev.name.replace(/\s+/g,"_"));
  ...
  async function handleShareAfter(){
    ...
    try{
      ...
      const cards=[buildStandingsCard(ciStands,tc,mp)];
      if(plan) cards.push(buildResultsTableCard(plan,ciStands,tc,mp));
      if(plan) cards.push(buildRoundResultsCard(plan));
      const result = await shareImages(cards, ev.name.replace(/\s+/g,"_")+"_results");
      if(result.status==="shared"){ onToast&&onToast("Shared ✓"); }
      else { onToast&&onToast("Native share unavailable — images downloaded"); setShareDiag(result.diag); }
    }catch(e){
      console.error("Share error:",e);
  ...
  return <><BBtn onBack={onBack} label="Back"/>
    ...
    <Card style={{padding:0,overflow:"hidden"}}>
      ...
      <div onClick={()=>{if(window.confirm("Repair duplicate event IDs?\n\nThis scans all events and reassigns new unique IDs to any duplicates found, without deleting any data. Safe to run anytime."))onRepairIds();}} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 16px",cursor:"pointer",borderBottom:"0.5px solid var(--po-bdr)"}}>
        <span style={{fontSize:18}}>🔧</span>
        <span style={{flex:1,fontSize:14,color:"var(--po-text)"}}>Repair Duplicate Event IDs</span>
        <span style={{color:"var(--po-dim)"}}>›</span>
      </div>
