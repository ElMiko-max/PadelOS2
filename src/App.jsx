...
function maxPossibleCI(plan){
  // Max theoretical score = winning every round on Court 1, or break points if on break
  if(!plan)return 0;
  const tc=plan.courts;
  let max=0;
  plan.rounds.forEach(()=>{ max+=courtPts(1,tc); });
  return max;
}
...
const INIT_VENUES = [
  {id:1, name:"Galleria Moon Valley", gov:"القاهرة", area:"القاهرة الجديدة", courts:[{name:"Court 1"},{name:"Court 2"},{name:"Court 3"}], pricePerHour:0, extraFee:0, mapsUrl:"", status:"approved"},
];
...
const CARD_W = 720; // comfortable mobile width, scales cleanly when viewed full-size on a phone
const COLORS = {
  bg: "#F4F4FF",
  headerFrom: "#4F46E5",
  headerTo: "#7C6FF0",
...
function drawHeader(ctx, w, title, subtitle, communityName){
  ...
  ctx.font = "700 22px Arial"; ctx.fillText("🎾 PadelOS", 28, 38);
  if(communityName){ ctx.font="600 13px Arial"; ctx.fillStyle="#E0E7FF"; ctx.textAlign="right"; ctx.fillText(communityName, w-28, 38); ctx.textAlign="left"; }
  ctx.fillStyle = "#fff"; ctx.font = "700 26px Arial";
  ctx.fillText(title, 28, 78);
  if(subtitle){ ctx.font="500 14px Arial"; ctx.fillStyle="#E0E7FF"; ctx.fillText(subtitle, 28, 104); }
  return 140;
}

function drawFooter(ctx, w, h){
  ctx.fillStyle = COLORS.dim; ctx.font="11px Arial"; ctx.textAlign="center";
  ctx.fillText("Generated by PadelOS", w/2, h-16);
  ctx.textAlign="left";
}
...
async function shareImages(canvases, baseName){
  ...
  for(let i=0;i<canvases.length;i++){
    ...
    if(!blob) throw new Error("Failed to generate image "+(i+1));
    files.push(new File([blob],`${baseName}_${i+1}.png`,{type:"image/png"}));
  }
  diag.push(`navigator.share: ${typeof navigator.share}`);
  diag.push(`navigator.canShare: ${typeof navigator.canShare}`);
  // Try native share with all files at once
  if(navigator.share && navigator.canShare){
    ...
    if(canMulti){
      try{
        ...
        if(e && e.name==="AbortError") return {status:"shared", diag:["user cancelled share sheet"]};
        diag.push(`share(multi) threw: ${e.name}: ${e.message}`);
      }
    }
  } else {
    diag.push("Web Share API not available in this browser");
  }
  // Try sharing files one at a time (some Android browsers reject multi-file but accept single-file)
  if(navigator.share && navigator.canShare){
    let anyShared=false;
    for(const f of files){
      let canOne=false;
      try{ canOne = navigator.canShare({files:[f]}); }catch(e){ diag.push(`canShare(1 file) threw: ${e.message}`); }
      if(canOne){
        try{
          ...
          diag.push(`share(1 file ${f.name}) threw: ${e.name}: ${e.message}`);
        }
      } else {
        diag.push(`canShare returned false for ${f.name}`);
      }
    }
    if(anyShared) return {status:"shared", diag};
  }
  // Fallback: trigger downloads (native share unavailable on this browser)
  files.forEach(f=>{
...
function buildEventInfoCard(ev, venue, players, communityName){
  const w = CARD_W;
  const rows = Math.ceil(players.length/2);
  const h = 140 + 270 + rows*46 + 90; // header + info block + player grid + footer
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  drawHeader(ctx, w, ev.name, `${fmtD(ev.date)} · ${ev.time}${ev.timeTo?" → "+ev.timeTo:""}`, communityName);

  let y = 168;
  // Info card
  ctx.fillStyle = COLORS.card; roundRect(ctx, 24, y-26, w-48, 230, 14); ctx.fill();
  ctx.strokeStyle = COLORS.border; ctx.lineWidth=1; roundRect(ctx, 24, y-26, w-48, 230, 14); ctx.stroke();
  y += 4;
  const infoRows = [
    ["📍 Location", venue ? `${venue.name} — ${venue.area}` : "TBD"],
  ...
  infoRows.forEach(([label,val])=>{
    ctx.fillStyle = COLORS.dim; ctx.fillText(label, 40, y+14);
    ctx.fillStyle = COLORS.text; ctx.font="600 13px Arial"; ctx.textAlign="right";
    ctx.fillText(val, w-40, y+14); ctx.textAlign="left"; ctx.font="13px Arial";
    y += 38;
  });
  y += 14;

  // Late-arrival warning
  ctx.fillStyle = "#FEF3C7"; roundRect(ctx, 24, y-18, w-48, 50, 10); ctx.fill();
  ctx.fillStyle = COLORS.amber; ctx.font="600 12px Arial";
  y = wrapText(ctx, "⏰ Please arrive on time — late arrivals disrupt the round schedule for everyone.", 38, y, w-76, 17);
  y += 18;

  // Players header
  ctx.fillStyle = COLORS.text; ctx.font="700 16px Arial";
  ctx.fillText(`Registered Players (${players.length})`, 24, y); y += 18;

  const sorted = [...players].sort((a,b)=>b.usr-a.usr);
  const colW = (w-48-10)/2;
  sorted.forEach((p,i)=>{
    const col=i%2, row=Math.floor(i/2);
    const x = 24 + col*(colW+10), yy = y + row*46;
    ctx.fillStyle = COLORS.card; roundRect(ctx, x, yy, colW, 38, 10); ctx.fill();
    ctx.strokeStyle = COLORS.border; roundRect(ctx, x, yy, colW, 38, 10); ctx.stroke();
    // avatar circle with initials
    const lv = usrLv(p.usr);
    ctx.fillStyle = lv.c+"33"; ctx.beginPath(); ctx.arc(x+22, yy+19, 14, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = lv.c; ctx.font="700 11px Arial"; ctx.textAlign="center";
    ctx.fillText((p.avatar||ini2(p.nickname)), x+22, yy+23); ctx.textAlign="left";
    ctx.fillStyle = COLORS.text; ctx.font="600 13px Arial";
    ctx.fillText(p.nickname, x+44, yy+24);
    ctx.fillStyle = COLORS.dim; ctx.font="11px Arial"; ctx.textAlign="right";
    ctx.fillText(`USR ${p.usr}`, x+colW-12, yy+24); ctx.textAlign="left";
  });
...
function buildBreaksRound1Card(plan,tc,communityName){
  const w = CARD_W;
  const r1 = plan.rounds[0];
  const h = 140 + 70 + 50 + r1.matches.length*72 + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  drawHeader(ctx, w, "Break Schedule & Round 1", `${plan.totalRounds} rounds · ${tc} courts`, communityName);

  let y = 168;
  ctx.fillStyle = COLORS.card; roundRect(ctx, 24, y-26, w-48, 64, 12); ctx.fill();
  ctx.strokeStyle = COLORS.border; roundRect(ctx, 24, y-26, w-48, 64, 12); ctx.stroke();
  ctx.fillStyle = COLORS.amber; ctx.font="700 13px Arial";
  ctx.fillText("☕ Round 1 — On Break", 40, y);
  ctx.fillStyle = COLORS.text; ctx.font="600 14px Arial";
  ctx.fillText((r1.onBreak||[]).map(p=>p.nickname).join(", ") || "Nobody", 40, y+24);
  y += 70;

  ctx.fillStyle = COLORS.text; ctx.font="700 16px Arial";
  ctx.fillText("Round 1 — Matches", 24, y); y += 16;

  r1.matches.forEach(m=>{
    ctx.fillStyle = COLORS.card; roundRect(ctx, 24, y, w-48, 60, 12); ctx.fill();
    ctx.strokeStyle = COLORS.border; roundRect(ctx, 24, y, w-48, 60, 12); ctx.stroke();
    ctx.fillStyle = COLORS.accent; ctx.font="700 12px Arial";
    ctx.fillText(`COURT ${m.court}`, 40, y+22);
    ctx.fillStyle = COLORS.text; ctx.font="600 14px Arial";
    const teamA = m.teamA.map(p=>p.nickname).join(" & ");
    const teamB = m.teamB.map(p=>p.nickname).join(" & ");
    ctx.fillText(teamA, 40, y+44);
    ctx.fillStyle = COLORS.dim; ctx.font="600 12px Arial";
    ctx.fillText("vs", 40 + ctx.measureText(teamA).width + 10, y+44);
    ctx.fillStyle = COLORS.text; ctx.font="600 14px Arial";
    ctx.fillText(teamB, 40 + ctx.measureText(teamA).width + 34, y+44);
    y += 72;
  });
...
function buildStandingsCard(ciStands,tc,maxPts,communityName){
  const w = CARD_W;
  const h = 140 + 60 + ciStands.length*60 + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  drawHeader(ctx, w, "Final Standings", `Max possible: ${maxPts||"—"} pts`, communityName);
  ...
  ciStands.forEach((s,i)=>{
    ...
    roundRect(ctx, 24, y, w-48, 50, 12); ctx.fill();
    ctx.strokeStyle = isTop ? "#FDE68A" : COLORS.border;
    roundRect(ctx, 24, y, w-48, 50, 12); ctx.stroke();

    ctx.fillStyle = isTop ? COLORS.amber : i===1?"#64748B":i===2?"#B45309":COLORS.text;
    ctx.font = "700 15px Arial";
    ctx.fillText(`${isTop?"🏆":i+1+"."} ${s.user.nickname}`, 40, y+22);
    ctx.fillStyle = COLORS.dim; ctx.font="11px Arial";
    ctx.fillText(`${s.wins}W · ${s.breaks} breaks · PES ${pes}%`, 40, y+39);

    ctx.fillStyle = COLORS.accent; ctx.font="700 19px Arial"; ctx.textAlign="right";
    ctx.fillText(`${s.pts}`, w-40, y+28);
    ctx.fillStyle = COLORS.dim; ctx.font="10px Arial";
    ctx.fillText("pts", w-40, y+42); ctx.textAlign="left";
    y += 60;
  });
...
function buildResultsTableCard(plan,ciStands,tc,maxPts,communityName){
  const colW=64, nameW=160, pesW=56, totalW=56;
  const w = Math.max(CARD_W, nameW+plan.rounds.length*colW+pesW+totalW+48);
  const h = 140 + 50 + ciStands.length*36 + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  drawHeader(ctx, w, "Match Results Table", `${plan.rounds.length} rounds · Max ${maxPts||"—"} pts`, communityName);

  let y = 168;
  ctx.fillStyle = COLORS.dim; ctx.font="700 11px Arial";
  ctx.fillText("PLAYER", 28, y);
  plan.rounds.forEach((r,ri)=>ctx.fillText(`M${ri+1}`, 28+nameW+ri*colW+22, y));
  ctx.fillText("PES", 28+nameW+plan.rounds.length*colW+12, y);
  ctx.fillText("PTS", 28+nameW+plan.rounds.length*colW+pesW+10, y);
  y += 14;
  ctx.strokeStyle = COLORS.border; ctx.beginPath(); ctx.moveTo(24,y); ctx.lineTo(w-24,y); ctx.stroke();
  y += 22;
  ...
  ciStands.forEach((s,i)=>{
    ...
    roundRect(ctx, 24, y-18, w-48, 30, 8); ctx.fill();
    ctx.fillStyle = COLORS.text; ctx.font="600 12px Arial";
    ctx.fillText(`${i+1}. ${s.user.nickname} (${s.user.usr})`, 32, y);
    plan.rounds.forEach((r,ri)=>{
      ...
      ctx.font = cell.win ? "700 11px Arial" : "11px Arial";
      ctx.fillText(cell.text, 28+nameW+ri*colW+22, y);
    });
    ctx.fillStyle = COLORS.sub; ctx.font="600 11px Arial";
    ctx.fillText(`${pes}%`, 28+nameW+plan.rounds.length*colW+12, y);
    ctx.fillStyle = COLORS.accent; ctx.font="700 12px Arial";
    ctx.fillText(`${s.pts}`, 28+nameW+plan.rounds.length*colW+pesW+14, y);
    y += 36;
  });
...
function buildRoundResultsCard(plan,communityName){
  const w = CARD_W;
  let estH = 140;
  plan.rounds.forEach(r=>{ estH += 36 + r.matches.length*54; });
  const h = estH + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  drawHeader(ctx, w, "Round-by-Round Results", `${plan.rounds.length} rounds played`, communityName);

  let y = 168;
  plan.rounds.forEach((r,ri)=>{
    ctx.fillStyle = COLORS.text; ctx.font="700 15px Arial";
    ctx.fillText(`Round ${ri+1}`, 24, y); y += 24;
    r.matches.forEach(m=>{
      const teamA = m.teamA.map(p=>p.nickname).join(" & ");
      const teamB = m.teamB.map(p=>p.nickname).join(" & ");
      const winT = m.winner==="A"?teamA:m.winner==="B"?teamB:null;
      ctx.fillStyle = COLORS.card; roundRect(ctx, 24, y-16, w-48, 44, 10); ctx.fill();
      ctx.strokeStyle = COLORS.border; roundRect(ctx, 24, y-16, w-48, 44, 10); ctx.stroke();
      ctx.fillStyle = COLORS.dim; ctx.font="600 11px Arial";
      ctx.fillText(`Court ${m.court}`, 38, y-2);
      ctx.fillStyle = COLORS.sub; ctx.font="12px Arial";
      ctx.fillText(`${teamA}  vs  ${teamB}`, 38, y+16);
      if(winT){ ctx.fillStyle = COLORS.green; ctx.font="700 11px Arial"; ctx.textAlign="right"; ctx.fillText(`✓ ${winT}`, w-38, y+5); ctx.textAlign="left"; }
      y += 54;
    });
    y += 10;
  });
...
function BBtn({onBack,label="Back",sticky=false,subLabel}){
  const content = <button onClick={onBack} className="po-dim" style={{display:"flex",alignItems:"center",gap:4,background:"none",border:"none",color:"var(--po-dim)",fontSize:14,fontWeight:500,cursor:"pointer",padding:"10px 0",minHeight:40}}>← {label}{subLabel?<span style={{color:"var(--po-sub)"}}>&nbsp;({subLabel})</span>:null}</button>;
  if(!sticky) return <div style={{marginBottom:8}}>{content}</div>;
  return <div style={{position:"sticky",top:64,zIndex:40,background:"var(--po-bg)",marginLeft:-12,marginRight:-12,paddingLeft:12,paddingRight:12,marginBottom:8,borderBottom:"0.5px solid var(--po-bdr)"}}>{content}</div>;
}
...
export default function PadelOS() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        // Recompute global ID counters from actual loaded data so new IDs are always unique,
        // even across page reloads (module-level counters reset to their initial values on every load).
        if (d.users)  _uid = Math.max(_uid,  ...d.users.map(u=>u.id), 0) + 1;
        if (d.venues) _vid = Math.max(_vid,  ...d.venues.map(v=>v.id), 0) + 1;
        if (d.comms)  _cid = Math.max(_cid,  ...d.comms.map(c=>c.id), 0) + 1;
        if (d.comms)  _eid = Math.max(_eid,  ...d.comms.flatMap(c=>c.events.map(e=>e.id)), 0) + 1;
      }
  ...
  const duplicateEvent=(cid,eid,newDate,keepPlayers)=>{
  ...
  const closeEvent=(cid,eid)=>{
    ...
    // Calculate PES for each player and update USR
    if(ev.type==="closed_ind"&&ev.plan){
      const plan=ev.plan, tc=plan.courts;
      ...
      setUsers(us=>us.map(u=>{
        ...
        const hist=[...(u.usrHistory||[]), {eventId:eid, eventName:ev.name, date:ev.date, pes}];
        const last5=hist.slice(-5);
        const newUsr=Math.round(last5.reduce((sum,h)=>sum+h.pes,0)/last5.length);
        return {...u, usr:newUsr, usrHistory:hist};
      }));
    }
    updC(cid,c=>({...c,events:c.events.map(ev=>ev.id!==eid?ev:{...ev,status:"completed",closedAt:new Date().toISOString()})}));
    toast2("Event closed ✓ — USR updated");
  };
  ...
  // CT
  const startCT=(cid,eid,courts,fmt)=>{const ev=getEv(cid,eid);if(!ev)return;const players=ev.registrations.map(r=>{const u=users.find(u=>u.id===r.userId);if(!u)return null;return{...u,usr:r.eventUsr??u.usr,userId:r.userId};}).filter(Boolean);setPlan(cid,eid,generateCTPlan(players,courts,fmt,ev));toast2("Teams formed ✓");};
    ...
    <div onClick={()=>menu&&setMenu(false)}
      ...
      <div style={{flex:1,maxWidth:680,width:"100%",margin:"0 auto",padding:"16px 12px 80px"}}>
        ...
        {nav==="communities"&&view.screen==="event"&&comm&&event&&
          <EvDetail ev={event} comm={comm} users={users} venues={venues} me={me} onToast={msg=>toast2(msg)}
            onDuplicate={(newDate,keepPlayers)=>duplicateEvent(comm.id,event.id,newDate,keepPlayers)}
...
function TopBar({me,nav,menu,setMenu,onNav,onProfile,onVenues,onSettings,TH,dark}){
  const tabs = [
    {k:"communities", l:"Communities", icon:(
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="9" cy="8" r="3" fill="#FBBF24"/><circle cx="17" cy="9" r="2.5" fill="#34D399"/><path d="M3 19c0-3 2.7-5 6-5s6 2 6 5" stroke="#FBBF24" strokeWidth="2" strokeLinecap="round"/><path d="M13.5 14.5c2.6 0 5 1.6 5 4.5" stroke="#34D399" strokeWidth="2" strokeLinecap="round"/></svg>
    )},
    {k:"events", l:"Events", icon:(
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="3" y="5" width="18" height="16" rx="3" fill="#F472B6" fillOpacity="0.25" stroke="#F472B6" strokeWidth="2"/><path d="M3 10h18" stroke="#F472B6" strokeWidth="2"/><path d="M8 3v4M16 3v4" stroke="#F472B6" strokeWidth="2" strokeLinecap="round"/><circle cx="9" cy="15" r="1.4" fill="#38BDF8"/><circle cx="15" cy="15" r="1.4" fill="#38BDF8"/></svg>
    )},
  ];
  return <div style={{background:TH?.nav||"#0E1117",borderBottom:`0.5px solid ${TH?.border||"var(--po-bdr)"}`,padding:"0 14px",display:"flex",alignItems:"center",justifyContent:"space-between",height:64,position:"sticky",top:0,left:0,right:0,width:"100%",zIndex:50,transition:"all 0.2s",boxSizing:"border-box"}}>
    <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
      <div style={{width:32,height:32,background:"linear-gradient(135deg,#6366F1,#06B6D4)",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:"#fff"}}>PO</div>
      <span style={{fontSize:14,fontWeight:600,color:dark?"#F1F5F9":"#FFFFFF",display:"none"}} className="po-applogo-text">PadelOS</span>
    </div>
    <div style={{display:"flex",gap:4,flex:1,justifyContent:"center"}}>{tabs.map(t=>
      <button key={t.k} onClick={()=>onNav(t.k)} style={{display:"flex",alignItems:"center",gap:7,padding:"10px 16px",borderRadius:10,border:"none",fontSize:13,fontWeight:600,cursor:"pointer",minHeight:44,background:nav===t.k?(dark?"var(--po-bdr)":"rgba(255,255,255,0.22)"):"transparent",color:dark?(nav===t.k?"#F1F5F9":"var(--po-dim)"):"#FFFFFF",transition:"background 0.15s"}}>
        {t.icon}<span>{t.l}</span>
      </button>
    )}</div>
    <div style={{position:"relative",flexShrink:0}} onClick={e=>e.stopPropagation()}>
      <div onClick={()=>setMenu(o=>!o)} style={{cursor:"pointer",padding:4}}><Av u={me} size={36}/></div>
      {menu&&<div style={{position:"absolute",right:0,top:48,background:"var(--po-card)",border:"0.5px solid var(--po-bdr)",borderRadius:10,padding:6,minWidth:190,zIndex:100,boxShadow:"0 8px 32px #00000066"}}>
        ...
        {[{i:"👤",l:"My Profile",fn:onProfile},{i:"🏟",l:"Venues",fn:onVenues},{i:"⚙️",l:"Settings",fn:onSettings},{i:"🚪",l:"Sign Out",fn:()=>setMenu(false),d:true}].map(x=><button key={x.l} onClick={x.fn} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"10px 10px",minHeight:40,borderRadius:7,border:"none",background:"transparent",color:x.d?"#EF4444":"var(--po-sub)",fontSize:13,cursor:"pointer",textAlign:"left"}}>{x.i} {x.l}</button>)}
      </div>}
    </div>
  </div>;
}
...
function CommDetail({comm,users,me,onBack,onEdit,onApprove,onReject,onPromote,onKick,onInvite,onOpenEv,onCreateEv,onViewProfile}){
  ...
  return <><BBtn onBack={onBack} label="Communities" sticky subLabel={tab==="members"?"Members":tab==="events"?"Events":"Requests"}/>
    ...
    {tab==="events"&&<>{isAdmin&&<Btn label="+ New Event" primary onClick={onCreateEv} style={{width:"100%",marginBottom:12}}/>}
      {comm.events.length===0?<Card><div style={{textAlign:"center",color:"var(--po-dim)",fontSize:13,padding:"20px 0"}}>No events yet</div></Card>:<>
        {(() => {
          const upcoming=comm.events.filter(ev=>ev.status!=="completed"&&ev.status!=="cancelled"&&!ev.archived);
          const past=comm.events.filter(ev=>(ev.status==="completed"||ev.status==="cancelled")&&!ev.archived);
          const archived=comm.events.filter(ev=>ev.archived);
          return <>
            {upcoming.length>0&&<>{upcoming.map(ev=><EvCard key={ev.id} ev={ev} onClick={()=>onOpenEv(ev.id)}/>)}</>}
            {upcoming.length===0&&<Card><div style={{textAlign:"center",color:"var(--po-dim)",fontSize:13,padding:"16px 0"}}>No upcoming events</div></Card>}
            {past.length>0&&<><ST>📁 Past Events ({past.length})</ST>{past.map(ev=><EvCard key={ev.id} ev={ev} onClick={()=>onOpenEv(ev.id)}/>)}</>}
            {isAdmin&&archived.length>0&&<><ST>📦 Archived ({archived.length})</ST>{archived.map(ev=><EvCard key={ev.id} ev={ev} onClick={()=>onOpenEv(ev.id)}/>)}</>}
          </>;
        })()}
      </>}
    </>}
...
function EventEditForm({ev,venues,onBack,onSave}){
  ...
  const lockedSchedule = isCompleted; // date/time/courts locked once completed — would corrupt historical results
  return <><BBtn onBack={onBack} label={ev.name}/><div className="po-text" style={{fontSize:18,fontWeight:600,color:"var(--po-text)",marginBottom:16}}>Edit Event</div>
    <Card>
      <div style={{fontSize:12,color:"var(--po-dim)",marginBottom:14,padding:"8px 12px",background:"var(--po-card)",borderRadius:8}}>ℹ️ {isCompleted?"This event is completed — only the name and description can be edited.":lockedType?"Type is locked — a plan has already been generated for this event.":"Players and plan stay unchanged unless you change the event type."}</div>
      ...
      <Btn label="Save Changes" primary onClick={()=>onSave(lockedSchedule?{name:f.name,description:f.description}:{name:f.name,description:f.description,date:f.date,courts:parseInt(f.courts),time:f.time,timeTo:f.timeTo,type:f.eventType})} style={{width:"100%"}}/>
    </Card>
  </>;
}
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive}){
  ...
  const startSim = () => {
    ...
    setSimSnapshot(snap);
    setSimEv(JSON.parse(JSON.stringify(ev)));
    setSim(true);
  ...
  const act = {
    ...
    startCT: (c,f) => sim
      ? simMutate(e => {
          ...
          return {...e, plan: generateCTPlan(players, c, f, e)};
        })
      : onStartCT(c,f),
    setWinCT: (...args) => sim ? null /* Closed Teams sim scoring not mirrored yet */ : onSetWinCT(...args),
    applyPromo: () => sim ? null : onApplyPromo(),
    nextCTLadder: () => sim ? null : onNextCTLadder(),
    swapCTLadder: (...args) => sim ? null : onSwapCTLadder(...args),
    closeEvent: () => sim
  ...
  const totC   = (effEv.costPerCourt||0)*tc+(effEv.extraFee||0)*tc;
  ...
  async function handleShareBefore(){
    ...
    try{
      const cards=[buildEventInfoCard(effEv,venue,sharePlayers,comm.name)];
      if(isCI&&plan&&plan.rounds[0]) cards.push(buildBreaksRound1Card(plan,tc,comm.name));
      const result = await shareImages(cards, effEv.name.replace(/\s+/g,"_"));
      if(result.status==="shared"){ onToast&&onToast("Shared ✓"); }
      else { onToast&&onToast("Native share unavailable — images downloaded"); setShareDiag(result.diag); }
    }catch(e){
  ...
  async function handleShareAfter(){
    setSharing(true);setShareDiag(null);
    try{
      const mp = plan ? maxPossibleCI(plan) : 0;
      const cards=[buildStandingsCard(ciStands,tc,mp,comm.name)];
      if(plan) cards.push(buildResultsTableCard(plan,ciStands,tc,mp,comm.name));
      if(plan) cards.push(buildRoundResultsCard(plan,comm.name));
  ...
  function PChip({p,ri}){const lv=usrLv(p.usr),isSel=sel?.ri===ri&&sel?.uid===p.userId,isTgt=sel&&sel.ri===ri&&sel.uid!==p.userId;return <div onClick={()=>!isCompleted&&tapP(ri,p.userId)} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:8,cursor:"pointer",userSelect:"none",border:`2px solid ${isSel?"#FBBF24":isTgt?"#34D399":"transparent"}`,background:isSel?"#FBBF2422":isTgt?"#34D39922":"transparent"}}><div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,background:`${lv.c}22`,border:`1.5px solid ${lv.c}66`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:lv.c}}>{p.avatar||ini2(p.nickname)}</div><span style={{fontSize:13,fontWeight:500,color:"var(--po-text)",flex:1}}>{p.nickname}</span><span style={{fontSize:11,color:"var(--po-dim)"}}>{p.usr}</span></div>;}
  ...
  return <>
    <BBtn onBack={onBack} label={comm.name} sticky subLabel={tLabels[tab]}/>
    ...
    {sim&&<div style={{marginBottom:12,padding:"10px 14px",background:"#6366F111",borderRadius:10,border:"0.5px solid #6366F155",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}><div><div style={{fontSize:12,fontWeight:600,color:"#A5B4FC"}}>🧪 Practice Session Active</div><div style={{fontSize:10,color:"var(--po-dim)"}}>All changes here are temporary</div></div><SmBtn label="Exit & Discard" onClick={exitSim} color="#EF4444"/></div>}

    <Card>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
        <div>
          <div className="po-text" style={{fontWeight:700,fontSize:17,color:"var(--po-text)",marginBottom:4,display:"flex",alignItems:"center",gap:8}}>{ev.name} <span style={{fontSize:11,fontWeight:500,color:"var(--po-dim)",background:"var(--po-inp)",padding:"2px 8px",borderRadius:6}}>#{ev.id}</span></div>
      ...
      {showDup&&<div style={{marginTop:-4,marginBottom:12,padding:"12px",background:"var(--po-inp)",borderRadius:10,border:"0.5px solid #F59E0B44"}}>
        <div style={{fontSize:12,fontWeight:600,color:"#F59E0B",marginBottom:8}}>⧉ Duplicate this event — pick a new date</div>
        ...
        <Btn label="Create Copy" primary onClick={()=>{if(dupDate){onDuplicate(dupDate,dupKeepPlayers);setShowDup(false);}}} style={{width:"100%"}}/>
    ...
    {/* PLAYERS */}
    {tab==="players"&&<>
      {isAdmin&&<><div style={{display:"flex",gap:6,marginBottom:10}}><Btn label="+ Add Member" onClick={()=>{setSAM(o=>!o);setSAG(false);}} style={{flex:1}}/>{!sim&&<Btn label="+ Add Guest" onClick={()=>{setSAG(o=>!o);setSAM(false);}} style={{flex:1}}/>}</div>
      ...
      <ST>Registered ({effEv.registrations.length})</ST>
      {effEv.registrations.map(r=>{
        const u=users.find(u=>u.id===r.userId);if(!u)return null;
        ...
        return <Card key={r.userId}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              {u.isGuest||r.isGuest
                ? <div style={{display:"flex",alignItems:"center",gap:6,marginTop:2}}>
                    ...
                    <input type="number" min="0" max="100" value={u.usr}
                      onChange={e=>act.editGuestUsr(u.id,e.target.value)}
                      className="po-inp"
                ...
                : isAdmin&&!effEv.plan
                  ? <div style={{display:"flex",alignItems:"center",gap:6,marginTop:2}}>
                      <span style={{fontSize:11,color:"var(--po-dim)"}}>USR</span>
                      <input type="number" min="0" max="100" value={r.eventUsr??u.usr}
                        onChange={e=>act.editEventUsr(u.id,e.target.value)}
                        className="po-inp"
            ...
            <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"flex-end",alignItems:"center"}}>
              ...
              {isAdmin&&!effEv.plan&&<SmBtn label="✕" onClick={()=>act.removeFromEvent(u.id)} color="#EF4444" style={{padding:"4px 8px",fontSize:11}}/>}
            </div>
          </div>
        </Card>;
      })}
    ...
    {tab==="manage"&&isAdmin&&isOpen&&<>
      ...
      <ST>Financial Summary</ST><Card>{[["Courts",tc],["Cost/court",`${effEv.costPerCourt||0} EGP`],["Total",`${totC} EGP`],["Checked in",cinCnt],["Per player",cinCnt>0?`${cpp} EGP`:"—"]].map(([k,val])=><div key={k} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"0.5px solid var(--po-bdr)"}}><span style={{fontSize:13,color:"var(--po-dim)"}}>{k}</span><span style={{fontSize:13,fontWeight:600,color:k==="Per player"?"#34D399":"var(--po-text)"}}>{val}</span></div>)}</Card>
    </>}
    ...
    {tab==="rounds"&&isCI&&isAdmin&&<>
      ...
      {plan&&<>
        ...
        <div style={{fontSize:12,padding:"9px 12px",borderRadius:8,marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,background:sel?"#FBBF2411":"var(--po-bdr)",border:`0.5px solid ${sel?"#FBBF2444":"#334155"}`}}>
          <span style={{color:sel?"#FBBF24":"var(--po-dim)"}}>{isCompleted?"🔒 Event completed — results locked":sel?`✋ ${users.find(u=>u.id===sel.uid)?.nickname} — tap another in Round ${sel.ri+1} to swap`:"💡 Tap player to select · tap another in same round to swap"}</span>
    ...
    {tab==="standings"&&isCI&&<>
      ...
      {plan&&<div style={{marginBottom:10,padding:"8px 12px",background:"#6366F122",border:"0.5px solid #6366F144",borderRadius:8,fontSize:12,color:"#A5B4FC",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span>🎯 Max possible: <b>{maxPossibleCI(plan)} pts</b> ({plan.rounds.length} rounds × {courtPts(1,tc)}pts)</span>
      </div>}
    ...
    {tab==="teams"&&isCT&&isAdmin&&<>
      {!plan&&<Card>
        ...
        <Btn label="🎯 Form Teams & Start" primary onClick={()=>act.startCT(selCtC,ctF)} style={{width:"100%"}}/>
      </Card>}
      {plan&&<>
    ...
    {/* CT MATCHES */}
    {tab==="matches"&&isCT&&isAdmin&&plan&&<CTMatchesTab plan={plan} onSetWinCT={act.setWinCT} onApplyPromo={act.applyPromo} onNextCTLadder={act.nextCTLadder} onSwapCTLadder={act.swapCTLadder}/>}
...
function ProfileSc({user,comms,onBack,viewedByAdmin}){
  ...
  const hist=[...(user.usrHistory||[])].reverse();
  ...
  <Card><div style={{display:"flex",gap:14,alignItems:"center",marginBottom:16}}><Av u={user} size={56}/><div><div style={{fontWeight:700,fontSize:18,color:"var(--po-text)"}}>{user.nickname}</div><div style={{fontSize:13,color:"var(--po-dim)"}}>{user.name}</div><div style={{fontSize:12,color:"var(--po-dim)"}}>📍 {user.area} · {user.gov}</div></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8}}>{[["USR",user.usr],["Level",<span style={{color:lv.c,fontWeight:700}}>{lv.l}</span>],["Communities",mine.length],["Events",ec]].map(([l,v])=><div key={l} className="po-inp" style={{background:"var(--po-inp)",borderRadius:8,padding:"8px 4px",textAlign:"center"}}><div style={{fontSize:15,fontWeight:700,color:"var(--po-text)"}}>{v}</div><div style={{fontSize:10,color:"var(--po-dim)",marginTop:1}}>{l}</div></div>)}</div></Card>
  <ST>📈 USR History</ST>
  {hist.length===0?<Card><div style={{textAlign:"center",color:"var(--po-dim)",fontSize:13,padding:"16px 0"}}>No event history yet.</div></Card>:<Card style={{padding:0,overflow:"hidden"}}>
    {hist.map((h,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderBottom:i<hist.length-1?"0.5px solid var(--po-bdr)":"none"}}>
      <div style={{flex:1}}>
        <div style={{fontSize:13,fontWeight:600,color:"var(--po-text)"}}>{h.eventName}</div>
        <div style={{fontSize:11,color:"var(--po-dim)"}}>{fmtD(h.date)}</div>
      </div>
      <div style={{textAlign:"right"}}>
        <div style={{fontSize:11,color:"var(--po-dim)"}}>PES</div>
        <div style={{fontSize:15,fontWeight:700,color:"#6366F1"}}>{h.pes}</div>
      </div>
    </div>)}
  </Card>}
  <ST>{viewedByAdmin?`${user.nickname}'s Communities`:"My Communities"}</ST>{mine.map(c=>{const m=c.members.find(m=>m.userId===user.id);return <Card key={c.id}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{fontSize:20}}>🏸</div><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13,color:"var(--po-text)"}}>{c.name}</div><div style={{fontSize:11,color:"var(--po-dim)"}}>{c.area}</div></div>{rBdg(m.role)}{sBdg(m.status)}</div></Card>;})}</>;
}
function SettingsSc({user,users,dark,onToggleDark,onAddUser,onEditUser,onDeleteUser,onExport,onRepairIds,onFactoryReset,onBack}){
    ...
    <Card>
      ...
      <div style={{marginBottom:10}}>
        {(users||[]).map(u=><div key={u.id}>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:"0.5px solid var(--po-bdr)"}}>
            <Av u={u} size={32}/>
            <div style={{flex:1}}>
              <div style={{fontSize:13,fontWeight:600,color:"var(--po-text)"}}>{u.nickname}</div>
              <div style={{fontSize:11,color:"var(--po-dim)"}}>{u.name} · {u.area}</div>
            </div>
            <span style={{fontSize:13,fontWeight:700,color:"#6366F1",minWidth:48}}>USR {u.usr}</span>
            <SmBtn label={editingId===u.id?"▲":"✏️"} onClick={()=>editingId===u.id?setEditingId(null):startEdit(u)} color="#6366F1"/>
            {u.id!==user.id&&<SmBtn label="🗑" onClick={()=>{if(window.confirm(`Remove ${u.nickname}?`))onDeleteUser(u.id);}} color="#EF4444"/>}
          </div>
          {editingId===u.id&&<div style={{padding:"10px 0",borderBottom:"0.5px solid var(--po-bdr)"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
              <div><div style={{fontSize:11,color:"var(--po-dim)",marginBottom:3}}>Nickname</div>
                <input className="po-inp" value={ef.nickname} onChange={e=>sete("nickname",e.target.value)} style={{width:"100%",padding:"7px 10px",borderRadius:8,border:"0.5px solid var(--po-bdr)",background:"var(--po-inp)",color:"var(--po-text)",fontSize:13}}/></div>
              <div><div style={{fontSize:11,color:"var(--po-dim)",marginBottom:3}}>Full Name</div>
                <input className="po-inp" value={ef.name} onChange={e=>sete("name",e.target.value)} style={{width:"100%",padding:"7px 10px",borderRadius:8,border:"0.5px solid var(--po-bdr)",background:"var(--po-inp)",color:"var(--po-text)",fontSize:13}}/></div>
            </div>
            <div style={{marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                <span style={{fontSize:11,color:"var(--po-dim)"}}>USR</span>
                <span style={{fontSize:13,fontWeight:700,color:"#6366F1"}}>{ef.usr}</span>
              </div>
              <input type="range" min="0" max="100" value={ef.usr} onChange={e=>sete("usr",e.target.value)} style={{width:"100%"}}/>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
              <Drp label="المحافظة" value={ef.gov} onChange={v=>{sete("gov",v);sete("area","");}} options={Object.keys(EGYPT).map(g=>({v:g,l:g}))}/>
              <Drp label="المنطقة" value={ef.area} onChange={v=>sete("area",v)} options={(EGYPT[ef.gov]||[]).map(a=>({v:a,l:a}))}/>
            </div>
            <div style={{display:"flex",gap:8}}>
              <Btn label="✓ Save" primary onClick={()=>{if(ef.nickname){onEditUser(u.id,{...ef,usr:parseInt(ef.usr)||50});setEditingId(null);}}} style={{flex:1}}/>
              <Btn label="Cancel" onClick={()=>setEditingId(null)} style={{flex:1}}/>
            </div>
          </div>}
        </div>)}
      </div>
