...
function buildCTResultsTableCard(ev, venue, plan, ctStands, tc, communityName){
  ...
  // Calc max pts for TES
  const maxPts=rounds.length>0?rounds.reduce((sum,r)=>{
    return sum+ctLadderCourtPts(1,tc);
  },0):1;

  ctStands.forEach((s,si)=>{
    const tes=maxPts>0?Math.round((s.pts/maxPts)*100):0;
...
export default function PadelOS() {
  ...
  const startCT=(cid,eid,courts,fmt)=>{
    ...
    if(players.length%2!==0){
      // Odd count — most recently registered player goes to waiting list
      // Sort by registration time to find the last one added
      const regs=ev.registrations;
      const sorted=[...players].sort((a,b)=>{
        const ra=regs.find(r=>r.userId===a.userId||r.userId===a.id);
        const rb=regs.find(r=>r.userId===b.userId||r.userId===b.id);
        return new Date(rb?.registeredAt||0)-new Date(ra?.registeredAt||0);
      });
      waitlisted=sorted[0]; // most recently added
