  ...
  return <><BBtn onBack={onBack} label="Communities" sticky subLabel={tab==="members"?"Members":tab==="events"?"Events":"Requests"}/>
    ...
    <div style={{height:bannerHeight,borderRadius:"16px 16px 0 0",overflow:"hidden",position:"relative",padding:"0 16px",display:"flex",alignItems:"flex-end",background:comm.bannerURL?"#000":`linear-gradient(135deg, ${gradFrom}, ${gradTo})`,transition:"height 0.2s"}}>
      ...
      {/* Available to every community owner/admin — was Platform-Admin-only until confirmed
          working in production (see CHANGELOG V0.10.36/38). Deliberately real membership only
          (myRole, not isAdmin) — God Mode's platform-admin bypass shouldn't extend to editing
          the branding of a community you're not actually part of. */}
      {(myRole==="owner"||myRole==="admin")&&<div style={{position:"absolute",top:10,right:10,zIndex:2}} onClick={e=>e.stopPropagation()}>
