...
function ctEventMaxPts(plan){
  ...
  // Count total break slots across all rounds
  const totalBreakSlots = plan.rounds.reduce((sum,r)=>{
    return sum + ((r.onBreak||[]).length || (r.onBreakIds||[]).length || 0);
  }, 0);
  const typicalBreaks = Math.round(totalBreakSlots / numTeams);
  return (totalRounds - typicalBreaks) * c1 + typicalBreaks * bp;
}
...
const INIT_VENUES = [
  {id:1, name:"Galleria Moon Valley", gov:"القاهرة", area:"القاهرة الجديدة", courts:[{name:"Court 1"},{name:"Court 2"},{name:"Court 3"}], pricePerHour:500, extraFee:30, mapsUrl:"", status:"approved"},
];
...
let _uid=13,_cid=2,_eid=4,_vid=2,_nid=1;
...
function playWhistle(){
  try{
    ...
    osc.type = "square";
    for(let i=0;i<8;i++) osc.frequency.setValueAtTime(i%2===0?2600:2200, now+i*0.09);
    gain.gain.setValueAtTime(0.0001, now);
    gain.gain.exponentialRampToValueAtTime(0.35, now+0.02);
    gain.gain.setValueAtTime(0.35, now+0.68);
...
export default function PadelOS() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (d.notifications) _nid = Math.max(_nid, ...d.notifications.map(n=>n.id), 0) + 1;
      }
  ...
  useEffect(() => {
    try {
      localStorage.setItem('padelos_v10', JSON.stringify({comms, users, venues, notifications, dark}));
    } catch(e) { console.log('Save error', e); }
  }, [comms, users, venues, notifications, dark]);
...
function CTMatchesTab({plan,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,totalBookingMin,onSetMatchModeStart}){
  ...
  return <>
    ...
    {reversedRounds.map((round,revIdx)=>{
      ...
      return <div key={ri} style={{marginBottom:20,opacity:isLatest?1:0.7}}>
        ...
        {isLatest&&<MatchTimerWidget plan={plan} roundNum={round.roundNum} roundDuration={plan.roundDuration} totalRounds={plan.maxRounds} totalBookingMin={totalBookingMin} onStart={onSetMatchModeStart}/>}
        {isLeague
...
// ══════════════════════════════════════════════════════
function MatchTimerWidget({plan,roundNum,roundDuration,totalRounds,totalBookingMin,onStart}){
  ...
  const offsets = started ? computeRoundEndOffsets(totalRounds, roundDuration, totalBookingMin, delayMin) : null;
  const endAt   = started ? new Date(plan.matchModeStartAt).getTime() + (offsets[roundNum]||roundNum*roundDuration)*60000 : null;
  const remaining = started ? Math.max(0, Math.round((endAt-now)/1000)) : null;
  const isUp = started && remaining<=0;
  const restDur = started && offsets ? (offsets[2]!==undefined ? offsets[2]-offsets[1] : roundDuration) : roundDuration;
  const isCompressed = started && roundNum>1 && Math.round(restDur)<roundDuration;
  ...
  useEffect(() => {
    if (isUp && !firedRef.current) { firedRef.current = true; playWhistle(); }
    if (!isUp) firedRef.current = false; // re-arms if a new later round mounts a fresh instance
  }, [isUp]);
  ...
  if (!started) {
    return <Card style={{marginBottom:10,background:"#6366F111",border:"0.5px solid #6366F144"}}>
      ...
      <div style={{fontSize:11,color:"var(--po-dim)",marginBottom:8}}>Starting Round 1 now — how many minutes behind the booking time are we?</div>
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        <input type="number" min="0" value={delayInput} onChange={e=>setDelayInput(e.target.value)} className="po-inp"
          style={{width:70,background:"var(--po-inp)",border:"0.5px solid var(--po-bdr)",borderRadius:8,padding:"8px 10px",color:"var(--po-text)",fontSize:13,boxSizing:"border-box"}}/>
        <span style={{fontSize:12,color:"var(--po-dim)"}}>min late</span>
        <Btn label="Start Round 1 ▶" primary onClick={()=>onStart(parseInt(delayInput)||0)} style={{flex:1}}/>
      </div>
    </Card>;
  }
  ...
  return <Card style={{marginBottom:10,background:isUp?"#EF444422":"#6366F111",border:`0.5px solid ${isUp?"#EF444466":"#6366F144"}`}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div>
        <div style={{fontSize:11,color:isUp?"#EF4444":"#A5B4FC",fontWeight:600}}>{isUp?"⏰ Round time's up!":`⏱ Round ${roundNum} ends in`}</div>
        <div style={{fontSize:26,fontWeight:800,color:isUp?"#EF4444":"var(--po-text)",fontVariantNumeric:"tabular-nums"}}>{mm}:{ss}</div>
      ...
      {isUp&&<SmBtn label="🔔 Replay" onClick={playWhistle} color="#EF4444"/>}
    </div>
  </Card>;
}
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive,onViewProfile,onSwapCTBreak,onRegenCTBreaks,onToggleExempt,onTogglePaid,onUpdateEventFinance,onSetMatchModeStart}){
  ...
  // Financial model:
  // Total = (courtCost × courts × durationHours) + bookingFee (once) + extraExpenses (once)
  ...
  const bookingFee  = effEv.bookingFee||0;
  const extraExp    = effEv.extraExpenses||0;
  const totC        = Math.round(courtTotal + extraFeeTotal + bookingFee + extraExp);
  ...
  return <>
    ...
    <Card>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
        <div>
          <div className="po-text" style={{fontWeight:700,fontSize:17,color:"var(--po-text)",marginBottom:4,display:"flex",alignItems:"center",gap:8}}>{ev.name} <span style={{fontSize:11,fontWeight:500,color:"var(--po-dim)",background:"var(--po-inp)",padding:"2px 8px",borderRadius:6}}>#{ev.id}</span>{SEEDED_EVENT_IDS.has(ev.id)&&<SeedBadge/>}</div>
    ...
    {tab==="manage"&&isAdmin&&<>
      ...
      <Card>
        ...
        {[
          ...
          ["Booking Fee", "one-time", `${bookingFee} EGP`],
          ["Extra Expenses", "additional", `${extraExp} EGP`],
      ...
      {isAdmin&&<Card style={{marginBottom:8}}>
        ...
        <div style={{marginBottom:8}}>
          <div style={{fontSize:11,color:"var(--po-dim)",marginBottom:3}}>Booking fee (EGP) — one-time</div>
          <input type="number" defaultValue={effEv.bookingFee||0} className="po-inp"
            style={{width:"100%",background:"var(--po-inp)",border:"0.5px solid var(--po-bdr)",borderRadius:8,padding:"7px 10px",color:"var(--po-text)",fontSize:13,boxSizing:"border-box"}}
            onBlur={e=>act.updateFinance({bookingFee:parseFloat(e.target.value)||0})}
            onKeyDown={e=>e.key==="Enter"&&e.target.blur()}/>
        </div>
        <div>
          <div style={{fontSize:11,color:"var(--po-dim)",marginBottom:3}}>Extra expenses (EGP) — balls, water, etc.</div>
      ...
      {/* Settlement — who's collecting, and who has paid them */}
      {isAdmin&&attCnt>0&&<>
        <ST>💵 Settlement</ST>
        ...
        {attendeeIds.filter(uid=>!exemptedIds.has(uid)).map(uid=>{
          const u=users.find(u=>u.id===uid); if(!u) return null;
          const isPayer = uid===payerId;
          const isPaid = isPayer || paidIds.has(uid);
          return <Card key={uid} style={{marginBottom:6,background:isPaid?"#34D39911":"var(--po-card)",borderColor:isPaid?"#34D39944":"var(--po-bdr)"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <Av u={u} size={32}/>
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:"var(--po-dim)"}}>{isPayer?"Collects from the rest":`${cpp} EGP`}</div>
              </div>
              {!isPayer&&<div onClick={()=>act.togglePaid(uid)} style={{padding:"6px 14px",borderRadius:8,background:isPaid?"#34D39922":"var(--po-inp)",border:`0.5px solid ${isPaid?"#34D39966":"var(--po-bdr)"}`,fontSize:12,fontWeight:600,color:isPaid?"#34D399":"var(--po-dim)",cursor:"pointer"}}>
      ...
      {/* Exemptions — admin only, only if there are attendees to split cost across */}
      {isAdmin&&attCnt>0&&<>
        <ST>Payment Exemptions</ST>
        <div style={{fontSize:11,color:"var(--po-dim)",marginBottom:8}}>Tap a player to toggle their payment exemption. Exempted players are excluded from the cost split.</div>
        {attendeeIds.map(uid=>{
          const u=users.find(u=>u.id===uid); if(!u) return null;
          const isEx=exemptedIds.has(uid);
          return <Card key={uid} style={{marginBottom:6,background:isEx?"#34D39911":"var(--po-card)",borderColor:isEx?"#34D39944":"var(--po-bdr)"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <Av u={u} size={32}/>
              <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"var(--po-text)"}}>{u.nickname}</div></div>
              <div onClick={()=>act.toggleExempt(uid)} style={{padding:"6px 14px",borderRadius:8,background:isEx?"#34D39922":"var(--po-inp)",border:`0.5px solid ${isEx?"#34D39966":"var(--po-bdr)"}`,fontSize:12,fontWeight:600,color:isEx?"#34D399":"var(--po-dim)",cursor:"pointer"}}>
                {isEx?"✓ Exempt":"Paying"}
              </div>
            </div>
          </Card>;
      ...
      {plan&&<>
        ...
        {[...plan.rounds].reverse().map((round,revIdx)=>{
          ...
          return <div key={ri} style={{marginBottom:24,opacity:isLatest?1:0.75}}>
            ...
            {isLatest&&!isCompleted&&<MatchTimerWidget plan={plan} roundNum={round.round} roundDuration={plan.roundDuration||roundDur} totalRounds={plan.totalRounds} totalBookingMin={durationHrs*60} onStart={act.setMatchModeStart}/>}
    ...
    {/* CT MATCHES */}
    {tab==="matches"&&isCT&&isAdmin&&plan&&<CTMatchesTab plan={plan} onSetWinCT={act.setWinCT} onApplyPromo={act.applyPromo} onNextCTLadder={act.nextCTLadder} onSwapCTLadder={act.swapCTLadder} totalBookingMin={durationHrs*60} onSetMatchModeStart={act.setMatchModeStart}/>}
