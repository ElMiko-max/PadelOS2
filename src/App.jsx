...
function CTMatchesTab({plan,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,totalBookingMin,eventTime,sim,onSetMatchModeStart}){
  ...
  return <>
    ...
    {reversedRounds.map((round,revIdx)=>{
      ...
      return <div key={ri} style={{marginBottom:20,opacity:isLatest?1:0.7}}>
        ...
        {isLatest&&<MatchTimerWidget plan={plan} roundDuration={plan.roundDuration} totalRounds={plan.maxRounds} totalBookingMin={totalBookingMin} eventTime={eventTime} onStart={onSetMatchModeStart}/>}
        {isLeague
...
// ══════════════════════════════════════════════════════
function MatchTimerWidget({plan,roundDuration,totalRounds,totalBookingMin,eventTime,onStart}){
  ...
  return <Card style={{marginBottom:10,background:flash?"#EF444422":"#6366F111",border:`0.5px solid ${flash?"#EF444466":"#6366F144"}`}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div>
        <div style={{fontSize:11,color:flash?"#EF4444":"#A5B4FC",fontWeight:600}}>{flash?"⏰ Time's up!":endClock?`⏱ Round ${slot} ends at ${endClock}`:`⏱ Round ${slot} ends in`}</div>
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive,onViewProfile,onSwapCTBreak,onRegenCTBreaks,onToggleExempt,onTogglePaid,onUpdateEventFinance,onSetMatchModeStart}){
  ...
  return <>
      ...
      {plan&&<>
        ...
        {[...plan.rounds].reverse().map((round,revIdx)=>{
          ...
          return <div key={ri} style={{marginBottom:24,opacity:isLatest?1:0.75}}>
            ...
            {isLatest&&!isCompleted&&<MatchTimerWidget plan={plan} roundDuration={plan.roundDuration||roundDur} totalRounds={plan.totalRounds} totalBookingMin={durationHrs*60} eventTime={effEv.time} onStart={act.setMatchModeStart}/>}
    ...
    {/* CT MATCHES */}
    {tab==="matches"&&isCT&&isAdmin&&plan&&<CTMatchesTab plan={plan} onSetWinCT={act.setWinCT} onApplyPromo={act.applyPromo} onNextCTLadder={act.nextCTLadder} onSwapCTLadder={act.swapCTLadder} totalBookingMin={durationHrs*60} eventTime={effEv.time} sim={sim} onSetMatchModeStart={act.setMatchModeStart}/>}
