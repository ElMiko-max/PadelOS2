...
const CARD_W = 600; // narrower mobile-friendly width — less empty horizontal space, larger relative text
const COLORS = {
  bg: "#F4F4FF",
  headerFrom: "#4F46E5",
  headerTo: "#7C6FF0",
...
function buildEventInfoCard(ev, venue, players, communityName){
  ...
  infoRows.forEach(([label,val])=>{
    ...
    ctx.fillStyle = COLORS.text; ctx.font="700 14px Arial"; ctx.textAlign="right";
    ctx.fillText(val, w-40, y+14); ctx.textAlign="left"; ctx.font="14px Arial";
    y += 38;
  });
  y += 14;
...
function buildFullBreakTableCard(ev, venue, plan, tc, communityName){
  ...
  const nameW = 150, colW = 50, rowH = 36;
  const tableX = 24, tableTop0 = 168;
  const w = Math.max(CARD_W, nameW + totalRounds*colW + 48);
  const h = 140 + 60 + 40 + players.length*rowH + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  drawHeader(ctx, w, "Break Schedule", `${totalRounds} rounds · ${tc} courts`, communityName);
  ...
  ctx.fillStyle = COLORS.accent; roundRect(ctx, tableX, y, tableW, headerH, 8); ctx.fill();
  ctx.fillStyle = "#fff"; ctx.font="700 12px Arial"; ctx.textAlign="left";
  ctx.fillText("PLAYER", tableX+12, y+21);
  ctx.textAlign = "center";
  for(let ri=0; ri<totalRounds; ri++){
    ctx.fillText(`R${ri+1}`, tableX+nameW+ri*colW+colW/2, y+21);
  }
  ctx.textAlign = "left";
  y += headerH;
  ...
  players.forEach((p,i)=>{
    ...
    ctx.fillStyle = COLORS.text; ctx.font="600 13px Arial";
    ctx.fillText(p.nickname, tableX+12, y+23);
    for(let ri=0; ri<totalRounds; ri++){
      ...
      if(onBreak){
        ctx.globalAlpha = isPlanned ? 0.5 : 1;
        ctx.fillStyle = COLORS.amber+"33";
        ctx.beginPath(); ctx.arc(cx, y+rowH/2, 13, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = COLORS.amber; ctx.font="700 14px Arial"; ctx.textAlign="center";
        ctx.fillText("☕", cx, y+rowH/2+5);
        ctx.globalAlpha = 1;
      } else {
        ctx.fillStyle = "#CBD5E1"; ctx.font="12px Arial"; ctx.textAlign="center";
        ctx.fillText("·", cx, y+rowH/2+4);
      }
      ctx.textAlign = "left";
    }
    y += rowH;
  ...
  if(generatedCount < totalRounds){
    ...
    ctx.fillText(`Rounds ${generatedCount+1}–${totalRounds} are planned, not yet generated (shown lighter)`, tableX, y);
  }
...
function buildStandingsCard(ev,venue,ciStands,tc,maxPts,communityName){
  const w = CARD_W;
  const h = 140 + 60 + 50 + ciStands.length*68 + 60;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  ...
  ciStands.forEach((s,i)=>{
    ...
    roundRect(ctx, 20, y, w-40, 58, 14); ctx.fill();
    ctx.strokeStyle = isTop ? "#FDE68A" : COLORS.border;
    roundRect(ctx, 20, y, w-40, 58, 14); ctx.stroke();

    ctx.fillStyle = isTop ? COLORS.amber : i===1?"#64748B":i===2?"#B45309":COLORS.text;
    ctx.font = "700 20px Arial";
    ctx.fillText(`${isTop?"🏆":i+1+"."} ${s.user.nickname}`, 34, y+25);
    ctx.fillStyle = COLORS.dim; ctx.font="14px Arial";
    ctx.fillText(`${s.wins}W · ${s.breaks} breaks · PES ${pes}%`, 34, y+45);

    ctx.fillStyle = COLORS.accent; ctx.font="700 27px Arial"; ctx.textAlign="right";
    ctx.fillText(`${s.pts}`, w-34, y+32);
    ctx.fillStyle = COLORS.dim; ctx.font="12px Arial";
    ctx.fillText("pts", w-34, y+48); ctx.textAlign="left";
    y += 68;
  });
...
function buildResultsTableCard(ev,venue,plan,ciStands,tc,maxPts,communityName){
  ...
  const h = 140 + 60 + 50 + ciStands.length*40 + 60;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  ...
  ciStands.forEach((s,i)=>{
    const pes = maxPts>0 ? Math.round((s.pts/maxPts)*100*10)/10 : 0;
    ctx.fillStyle = i%2===0 ? COLORS.card : COLORS.cardAlt;
    roundRect(ctx, 24, y-19, w-48, 34, 9); ctx.fill();
    ...
    plan.rounds.forEach((r,ri)=>{
      ...
      ctx.font = cell.win ? "700 12px Arial" : "12px Arial";
      ctx.fillText(cell.text, 28+nameW+ri*colW+22, y);
    });
    ctx.fillStyle = COLORS.sub; ctx.font="700 12px Arial";
    ctx.fillText(`${pes}%`, 28+nameW+plan.rounds.length*colW+12, y);
    ctx.fillStyle = COLORS.accent; ctx.font="700 13px Arial";
...
function buildRoundResultsCard(ev,venue,plan,communityName){
  const w = CARD_W;
  let estH = 140 + 60;
  plan.rounds.forEach(r=>{ estH += 30 + r.matches.length*58; });
  const h = estH + 60;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  ...
  y = drawEventStrip(ctx, w, y, ev, venue);
  plan.rounds.forEach((r,ri)=>{
    ctx.fillStyle = COLORS.text; ctx.font="700 19px Arial";
    ctx.fillText(`Round ${ri+1}`, 20, y); y += 26;
    r.matches.forEach(m=>{
      const teamA = m.teamA.map(p=>p.nickname).join(" & ");
      const teamB = m.teamB.map(p=>p.nickname).join(" & ");
      const winT = m.winner==="A"?teamA:m.winner==="B"?teamB:null;
      ctx.fillStyle = COLORS.card; roundRect(ctx, 20, y-17, w-40, 50, 12); ctx.fill();
      ctx.strokeStyle = COLORS.border; roundRect(ctx, 20, y-17, w-40, 50, 12); ctx.stroke();
      ctx.fillStyle = COLORS.dim; ctx.font="700 12px Arial";
      ctx.fillText(`COURT ${m.court}`, 34, y-2);
      ctx.fillStyle = COLORS.text; ctx.font="600 16px Arial";
      ctx.fillText(`${teamA}  vs  ${teamB}`, 34, y+19);
      if(winT){ ctx.fillStyle = COLORS.green; ctx.font="700 13px Arial"; ctx.textAlign="right"; ctx.fillText(`✓ ${winT}`, w-34, y+5); ctx.textAlign="left"; }
      y += 58;
    });
    y += 8;
  });
