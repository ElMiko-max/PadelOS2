...
export default function PadelOS() {
  ...
  useEffect(() => {
    ...
    const updatedUsers = users.map(u => {
      ...
      completedCI.forEach(ev => {
        ...
        updatedUser.usrHistory.push({eventId: ev.id, eventName: ev.name, date: ev.date, pes});
        anyUpdate = true;
      });
      if (!anyUpdate && updatedUser.usrHistory === u.usrHistory) return u;
      // Recalculate USR from last 5 PES entries
      const last5 = updatedUser.usrHistory.slice(-5);
      if (last5.length === 0) return u;
      const newUsr = Math.round(last5.reduce((sum,h) => sum+h.pes, 0) / last5.length);
      return {...updatedUser, usr: newUsr};
    });

    if (anyUpdate) setUsers(updatedUsers);
  }, []); // runs once on mount, after INIT_USERS/INIT_COMMS are loaded into state
  ...
  // CT
  const startCT=(cid,eid,courts,fmt)=>{
    const ev=getEv(cid,eid);if(!ev)return;
    ...
    if(players.length%2!==0){
      players=[...players].sort((a,b)=>b.usr-a.usr);
      waitlisted=players.pop();
      toast2(`${waitlisted.nickname} moved to waiting list — need even number for team formation`,"err");
    }
    const newPlan={...generateCTPlan(players,courts,fmt,ev),waitlisted:waitlisted?[{userId:waitlisted.userId,nickname:waitlisted.nickname,usr:waitlisted.usr}]:[]};
      ...
      <div style={{flex:1,maxWidth:680,width:"100%",margin:"0 auto",padding:"16px 12px 80px"}}>
        ...
        {nav==="communities"&&view.screen==="event"&&comm&&event&&
          <EvDetail ev={event} comm={comm} users={users} venues={venues} me={me} onToast={msg=>toast2(msg)}
            ...
            onViewProfile={uid=>{setNav("profile");go("profile",{uid,backCid:comm.id});}}
        ...
        return <Card key={r.userId}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"flex-end",alignItems:"center"}}>
              ...
              {(r.isGuest||u.isGuest)&&<Bdg label="Guest" color="#F59E0B"/>}
      ...
      {plan&&<>
        <div style={{padding:"8px 12px",background:"#34D39911",border:"0.5px solid #34D39933",borderRadius:8,fontSize:12,color:"#34D399",marginBottom:12,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          {plan.rounds.length===0||!plan.rounds[0]?.matchesA?.some(m=>m.winner)
            ? <SmBtn label="🔄 Regenerate" onClick={()=>{if(window.confirm("Discard current team formation and start over?\n\nAll teams and the current Round 1 will be cleared. Registered players stay.\n\nThis cannot be undone."))act.startCT(plan.courts,plan.format);}} color="#F59E0B"/>
            : null}
        </div>
        {plan.format==="ladder"?<>
...
function ProfileSc({user,comms,onBack,viewedByAdmin}){
  ...
  const teamHist=mine.flatMap(c=>c.events.filter(ev=>
    ev.type==="closed_teams"&&ev.status==="completed"&&ev.plan&&
    ev.registrations.some(r=>r.userId===user.id)
  ).map(ev=>{
    const team=ev.plan.teams?.find(t=>t.players?.some(p=>p.userId===user.id));
    const stands=ev.plan?calcCTStandings(ev.plan):[];
    const pos=stands.findIndex(s=>s.team?.id===team?.id)+1;
    const partnerIds=team?.players?.filter(p=>p.userId!==user.id)||[];
    return {
      eventName:ev.name,date:ev.date,format:ev.plan.format,
      teamName:team?.name||"?",
      partners:partnerIds,
      position:pos||"?",
      totalTeams:stands.length,
    };
  ...
  {tab==="usr"&&<>
    ...
    :<Card style={{padding:0,overflow:"hidden"}}>
      <div style={{padding:"10px 14px",background:"var(--po-bdr)",display:"flex",gap:10}}>
        <span style={{flex:1,fontSize:11,fontWeight:700,color:"var(--po-dim)"}}>EVENT</span>
        <span style={{fontSize:11,fontWeight:700,color:"var(--po-dim)"}}>PES</span>
        <span style={{fontSize:11,fontWeight:700,color:"var(--po-dim)",width:40,textAlign:"right"}}>USR</span>
      </div>
      {usrHist.map((h,i)=>{
        const prevUsr = i<usrHist.length-1 ? Math.round(usrHist.slice(i+1).slice(0,5).reduce((s,x)=>s+x.pes,0)/Math.min(5,usrHist.length-i-1)) : null;
        const change = prevUsr!=null ? Math.round(h.pes)-prevUsr : null;
        return <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderBottom:i<usrHist.length-1?"0.5px solid var(--po-bdr)":"none"}}>
          <div style={{flex:1}}>
          ...
          <div style={{width:40,textAlign:"right"}}>
            {change!=null&&<span style={{fontSize:10,color:change>0?"#34D399":change<0?"#EF4444":"var(--po-dim)"}}>{change>0?"+":""}{change}</span>}
          </div>
        </div>;
      })}
    </Card>}
    <div style={{marginTop:8,padding:"8px 12px",background:"var(--po-card)",borderRadius:8,fontSize:11,color:"var(--po-dim)"}}>
      Current USR: <b style={{color:"var(--po-text)"}}>{user.usr}</b> · Rolling average of last 5 Closed Individuals events
    </div>
  </>}

  {tab==="teams"&&<>
    {teamHist.length===0?<Card><div style={{textAlign:"center",color:"var(--po-dim)",fontSize:13,padding:"16px 0"}}>No Closed Teams event history yet.</div></Card>
    :<Card style={{padding:0,overflow:"hidden"}}>
      {teamHist.map((h,i)=><div key={i} style={{padding:"12px 14px",borderBottom:i<teamHist.length-1?"0.5px solid var(--po-bdr)":"none"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:600,color:"var(--po-text)"}}>{h.eventName}</div>
            <div style={{fontSize:11,color:"var(--po-dim)"}}>{fmtD(h.date)} · {h.format==="ladder"?"Ladder":"League"}</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:16,fontWeight:700,color:"#6366F1"}}>#{h.position}</div>
            <div style={{fontSize:10,color:"var(--po-dim)"}}>of {h.totalTeams}</div>
          </div>
        </div>
        <div style={{fontSize:11,color:"var(--po-sub)"}}>
          {h.teamName}{h.partners.length>0?` · with ${h.partners.map(p=>p.nickname).join(" & ")}`:""}
        </div>
      </div>)}
    </Card>}
    <div style={{marginTop:8,padding:"8px 12px",background:"var(--po-card)",borderRadius:8,fontSize:11,color:"var(--po-dim)"}}>
      Team history from all Closed Teams events · TR (Team Rating) coming in a future update
    </div>
  </>}
