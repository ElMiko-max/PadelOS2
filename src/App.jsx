...
const CARD_SCALE = 2; // render at 2x resolution for crisp output on high-DPI phone screens; layout math stays in CSS-pixel units
...
function buildFullBreakTableCard(ev, venue, plan, tc, communityName){
  ...
  const nameW = 86, colW = 36, rowH = 28;
  const tableX = 14;
  const w = Math.max(CARD_W, nameW + totalRounds*colW + 28);
  const h = 108 + 16 + 48 + 26 + players.length*rowH + 30;
  const {c, ctx} = makeCard(w, h);
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
