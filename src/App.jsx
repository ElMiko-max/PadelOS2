...
export default function PadelOS() {
  ...
  const dataLoaded = ["comms","users","venues","notifications","claimRequests"].every(k => loadedKeys.includes(k));
  ...
  const allEvents=comms.flatMap(c=>c.events.map(ev=>({...ev,commName:c.name,communityId:c.id}))).filter(ev=>!ev.isDemo||me.id===1);
...
function CommDetail({comm,users,me,onBack,onEdit,onApprove,onReject,onPromote,onKick,onInvite,onOpenEv,onCreateEv,onViewProfile}){
  ...
  return <><BBtn onBack={onBack} label="Communities" sticky subLabel={tab==="members"?"Members":tab==="events"?"Events":"Requests"}/>
    ...
    {tab==="events"&&<>{isAdmin&&<Btn label="+ New Event" primary onClick={onCreateEv} style={{width:"100%",marginBottom:12}}/>}
      {(() => { const visEvents = comm.events.filter(ev=>!ev.isDemo||me.id===1);
