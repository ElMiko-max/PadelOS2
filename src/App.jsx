...
export default function PadelOS() {
  ...
  useEffect(() => {
    ...
    const updatedUsers = users.map(u => {
      ...
      // Recalculate USR using seed-padded history (gradual changes for new players)
      const hist = updatedUser.usrHistory;
      if (hist.length === 0) return u;
      const seedUsr = u.seedUsr ?? u.usr;
      const padded = hist.length < 5
        ? [...Array(5-hist.length).fill({pes: seedUsr}), ...hist]
        : hist.slice(-5);
      const newUsr = Math.round(padded.reduce((sum,h) => sum+h.pes, 0) / padded.length);
  ...
  const closeEvent=(cid,eid)=>{
    ...
    if(ev.type==="closed_ind"&&ev.plan){
      ...
      setUsers(us=>us.map(u=>{
        ...
        const realCount = hist.length;
        const seedUsr = u.seedUsr ?? u.usr;
        const padded = realCount < 5
          ? [...Array(5-realCount).fill({pes: seedUsr}), ...hist]
          : hist.slice(-5);
        const newUsr = Math.round(padded.reduce((sum,h)=>sum+h.pes,0)/padded.length);
    ...
    if(ev.type==="closed_teams"&&ev.plan){
      ...
      stands.forEach(s=>{
        let tes=0;
        if(format==="ladder"){
          // Ladder: pts ÷ max possible pts × 100
          const maxRoundsPlayed=plan.rounds.length;
          const maxPts=maxRoundsPlayed>0?plan.rounds.reduce((sum,r)=>{
            const onBreak=(r.onBreak||[]).some(t=>t.id===s.team?.id);
            return sum+(onBreak?ctLadderBreakPts(plan.courts):ctLadderCourtPts(1,plan.courts));
          },0):1;
      ...
      setUsers(us=>us.map(u=>{
        ...
        newEntry.tr=newTr; // attach the computed TR to this entry

        // Replace entire history for this combination + keep other combinations
        const otherHistory=prevHistory.filter(h=>h.comboKey!==comboKey);
        return {...u, teamsHistory:[...otherHistory,...comboHist]};
      }));
    }
  ...
  const regenCTBreaks=(cid,eid)=>{
    ...
    // Rebuild break plan using the same logic as buildCTBreakPlan for remaining rounds
    const newBreakPlan=[...plan.breakPlan];
    const teams=plan.sorted||plan.teams;
    const tc=plan.courts;
    const total=plan.maxRounds||plan.breakPlan.length;
    // Re-run break allocation for ungenerated rounds only
    const fresh=buildCTBreakPlan(teams,tc,total);
  ...
  return (
    <div onClick={()=>menu&&setMenu(false)}
      ...
      `}</style>
      <TopBar me={me} nav={nav} menu={menu} setMenu={setMenu} TH={TH} dark={dark} onNav={n=>{goRoot(n);}} onProfile={()=>{goRoot("profile");setMenu(false);}} onVenues={()=>{goRoot("venues");setMenu(false);}} onSettings={()=>{goRoot("settings");setMenu(false);}}/>
      <div style={{flex:1,maxWidth:680,width:"100%",margin:"0 auto",padding:"16px 12px 80px"}}>
        ...
        {nav==="communities"&&view.screen==="comm"&&comm&&<CommDetail comm={comm} users={users} me={me} onBack={goBack} onEdit={()=>go("editComm",{cid:comm.id})} onApprove={uid=>approveReq(comm.id,uid)} onReject={uid=>rejectReq(comm.id,uid)} onPromote={uid=>promoteM(comm.id,uid)} onKick={uid=>kickM(comm.id,uid)} onInvite={uid=>inviteUser(comm.id,uid)} onOpenEv={eid=>go("event",{cid:comm.id,eid})} onCreateEv={()=>go("createEvent",{cid:comm.id})} onViewProfile={uid=>{setNav("profile");setNavHistory(h=>[...h,{nav,view}]);setView({screen:"profile",uid,backCid:comm.id});}}/>}
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive,onViewProfile,onSwapCTBreak,onRegenCTBreaks}){
  ...
  const act = {
    ...
    swapCTBreak: (ri,tA,tB) => sim ? null : onSwapCTBreak&&onSwapCTBreak(ri,tA,tB),
    regenCTBreaks: () => sim ? null : onRegenCTBreaks&&onRegenCTBreaks(),
    startCT: (c,f) => sim
      ? simMutate(e => {
  ...
  return <>
    ...
    {tab==="standings"&&isCT&&<>
      ...
      {ctStands.length===0
        ...
        : <>
          {/* Max possible score for Ladder */}
          {plan?.format==="ladder"&&plan?.rounds?.length>0&&(()=>{
            const maxRoundsPlayed = plan.rounds.length;
            const maxPts = Array.from({length:maxRoundsPlayed},(_,ri)=>{
              const r=plan.rounds[ri];
              const onBreak=(r.onBreak||[]).length>0;
              if(onBreak) return ctLadderBreakPts(tc);
              return ctLadderCourtPts(1,tc); // max = win on court 1
            }).reduce((s,v)=>s+v,0);
            return <div style={{fontSize:11,color:"var(--po-dim)",marginBottom:8,textAlign:"right"}}>Max possible: <b style={{color:"var(--po-text)"}}>{maxPts} pts</b></div>;
          })()}
          ...
          {ctStands.map((s,i)=>{
            ...
            const maxPts = plan?.format==="ladder" ? Array.from({length:maxRoundsPlayed},(_,ri)=>{
              const r=plan.rounds[ri]; const onBreak=(r.onBreak||[]).length>0;
              return onBreak?ctLadderBreakPts(tc):ctLadderCourtPts(1,tc);
            }).reduce((a,v)=>a+v,0) : 0;
            const tes = plan?.format==="ladder"&&maxPts>0 ? Math.round((s.pts/maxPts)*100) : null;
...
function ProfileSc({user,comms,onBack,viewedByAdmin}){
  ...
  {tab==="usr"&&<>
    ...
    :<Card style={{padding:0,overflow:"hidden"}}>
      {/* Header */}
      <div style={{display:"grid",gridTemplateColumns:"72px 1fr 44px 44px",gap:4,padding:"8px 12px",background:"var(--po-bdr)",fontSize:10,fontWeight:700,color:"var(--po-dim)"}}>
        <span>DATE</span><span>EVENT</span><span style={{textAlign:"right"}}>PES</span><span style={{textAlign:"right"}}>USR Δ</span>
      </div>
      {usrHist.map((h,i)=>{
        // Calculate prev USR using seed-padding (same formula as closeEvent)
        const seedUsr = user.seedUsr ?? user.usr;
        const histChron = [...usrHist].reverse(); // oldest first
        const idx = histChron.findIndex(x=>x.eventId===h.eventId);
        const histUpToNow = histChron.slice(0, idx); // events BEFORE this one
        const paddedPrev = histUpToNow.length < 5
          ? [...Array(5-histUpToNow.length).fill({pes:seedUsr}), ...histUpToNow]
          : histUpToNow.slice(-5);
        const prevUsr = histUpToNow.length>0 ? Math.round(paddedPrev.reduce((s,x)=>s+x.pes,0)/5) : seedUsr;
        const paddedNow = [...histUpToNow,h].length < 5
          ? [...Array(5-histUpToNow.length-1).fill({pes:seedUsr}), ...histUpToNow, h]
          : [...histUpToNow, h].slice(-5);
        const newUsr = Math.round(paddedNow.reduce((s,x)=>s+x.pes,0)/paddedNow.length);
        const delta = newUsr - prevUsr;
        return <div key={i} style={{display:"grid",gridTemplateColumns:"72px 1fr 44px 44px",gap:4,padding:"10px 12px",borderBottom:i<usrHist.length-1?"0.5px solid var(--po-bdr)":"none",alignItems:"center"}}>
          <div style={{fontSize:10,color:"var(--po-dim)"}}>{fmtD(h.date)}</div>
          <div>
            <div style={{fontSize:11,color:"var(--po-dim)",marginBottom:1}}>#{h.eventId}</div>
            <div style={{fontSize:13,fontWeight:600,color:"var(--po-text)"}}>{h.eventName}</div>
          </div>
          <div style={{textAlign:"right",fontSize:14,fontWeight:700,color:"#6366F1"}}>{h.pes}%</div>
          <div style={{textAlign:"right",fontSize:12,fontWeight:700,color:delta>0?"#34D399":delta<0?"#EF4444":"var(--po-dim)"}}>
    ...
    <div style={{marginTop:8,padding:"8px 12px",background:"var(--po-card)",borderRadius:8,fontSize:11,color:"var(--po-dim)"}}>
      Current USR: <b style={{color:"var(--po-text)"}}>{user.usr}</b> · Rolling average of last 5 events (seed: {user.seedUsr??user.usr})
    </div>
  </>}
