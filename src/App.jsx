...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive,onViewProfile,onSwapCTBreak,onRegenCTBreaks,onToggleExempt,onUpdateEventFinance}){
  ...
  const totC        = Math.round(courtTotal + bookingFee + extraExp);
  const cinCnt      = effEv.checkedIn.length;
  const payingCnt   = Math.max(0, cinCnt - exemptedIds.size);
  ...
  return <>
    ...
    {tab==="manage"&&isAdmin&&isOpen&&<>
      ...
      {sim&&cinCnt>0&&<Card style={{background:"#6366F111",border:"0.5px solid #6366F144",marginBottom:10}}><div style={{fontSize:13,fontWeight:600,color:"#A5B4FC",marginBottom:10}}>💰 Live Cost Settlement</div>{[["Total",`${totC} EGP`],["Checked In",cinCnt],["Paying",payingCnt],["Per Player",`${cpp} EGP`]].map(([k,val])=><div key={k} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"0.5px solid #6366F122"}}><span style={{fontSize:13,color:"var(--po-dim)"}}>{k}</span><span style={{fontSize:14,fontWeight:700,color:k==="Per Player"?"#A5B4FC":"var(--po-text)"}}>{val}</span></div>)}</Card>}
      <ST>💰 Financial</ST>
      <Card>
        {/* Cost breakdown */}
        {[
      ...
      {isAdmin&&<Card style={{marginBottom:8}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
          ...
          <div>
            <div style={{fontSize:11,color:"var(--po-dim)",marginBottom:3}}>Booking fee (EGP)</div>
      ...
      {/* Exemptions — admin only, only if anyone checked in */}
      {isAdmin&&cinCnt>0&&<>
