...
function EventForm({venues,onBack,onCreate}){
  const [f,setF]=useState({name:"",description:"",date:"",time:"18:00",timeTo:"22:00",venueId:"",courts:"2",rotationMin:"15",pollMode:false,eventType:"open",visibility:"public"});
...
function PlatformAdminSc({users,comms,venues,onBack,onAddUser,onEditUser,onDeleteUser,onViewProfile,claimRequests=[],onApproveClaim,onRejectClaim}){
  ...
  {tab==="users"&&<>
    ...
    {showAdd&&<Card style={{marginBottom:12}}>
      ...
      {[["Nickname","nickname"],["Full Name","name"],["Governorate","gov"],["Area","area"],["Phone","phone"]].map(([l,k])=>
  ...
  return <><BBtn onBack={onBack} label="Back"/>
    ...
    <Card style={{padding:0,overflow:"hidden"}}>
      ...
      {[{i:"🌍",l:"Language",n:"English"},{i:"📍",l:"Home Area",n:user.area}].map((item,i)=><div key={item.l} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 16px",borderBottom:i<1?"0.5px solid var(--po-bdr)":"none",cursor:"pointer"}}><span style={{fontSize:18}}>{item.i}</span><span style={{flex:1,fontSize:14,color:"var(--po-text)"}}>{item.l}</span><span style={{fontSize:12,color:"var(--po-dim)"}}>{item.n}</span><span style={{color:"var(--po-dim)"}}>›</span></div>)}
        ...
        <ST>Account</ST>
    <Card style={{padding:0,overflow:"hidden"}}>
      <div onClick={()=>editingId===user.id?setEditingId(null):startEdit(user)} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 16px",cursor:"pointer"}}>
        <span style={{fontSize:18}}>✏️</span>
        <span style={{flex:1,fontSize:14,color:"var(--po-text)"}}>Edit Profile</span>
        <span style={{color:"var(--po-dim)"}}>{editingId===user.id?"▲":"›"}</span>
      </div>
      {editingId===user.id&&<div style={{padding:"0 16px 16px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          ...
          <label style={{cursor:photoUploading?"default":"pointer"}}>
            ...
            <div style={{fontSize:11,color:"var(--po-dim)",marginTop:2}}>Pick from gallery or take a new one</div>
          </label>
        </div>
        <Inp label="Nickname" value={ef.nickname} onChange={v=>sete("nickname",v)}/>
        <Inp label="Phone" value={ef.phone} onChange={v=>sete("phone",v)}/>
        <div style={{display:"flex",gap:8,marginTop:4}}>
          <Btn label="Save" primary onClick={()=>{if(!ef.nickname.trim())return;onEditUser(user.id,{...ef,usr:user.usr});setEditingId(null);}} style={{flex:1}}/>
    ...
    <div style={{textAlign:"center",marginTop:24,fontSize:12,color:"var(--po-bdr)"}}>PadelOS {APP_VERSION}</div>
  </>;
}
