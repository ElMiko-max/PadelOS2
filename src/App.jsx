...
function buildBreaksRound1Card(ev,venue,plan,tc,communityName){
  const w = CARD_W;
  const r1 = plan.rounds[0];
  const h = 140 + 60 + 70 + 50 + r1.matches.length*78 + 70;
  const c = document.createElement("canvas"); c.width=w; c.height=h;
  const ctx = c.getContext("2d");
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  ...
  ctx.fillStyle = COLORS.card; roundRect(ctx, 24, y, w-48, 64, 14); ctx.fill();
  ctx.strokeStyle = COLORS.border; roundRect(ctx, 24, y, w-48, 64, 14); ctx.stroke();
  ctx.fillStyle = COLORS.amber; ctx.font="700 14px Arial";
  ctx.fillText("☕ Round 1 — On Break", 40, y+24);
  ctx.fillStyle = COLORS.text; ctx.font="600 16px Arial";
  ctx.fillText((r1.onBreak||[]).map(p=>p.nickname).join(", ") || "Nobody", 40, y+48);
  y += 80;

  ctx.fillStyle = COLORS.text; ctx.font="700 18px Arial";
  ctx.fillText("Round 1 — Matches", 24, y); y += 18;
...
function TopBar({me,nav,menu,setMenu,onNav,onProfile,onVenues,onSettings,TH,dark}){
  ...
  return <div style={{background:TH?.nav||"#0E1117",borderBottom:`0.5px solid ${TH?.border||"var(--po-bdr)"}`,padding:"0 12px",display:"flex",alignItems:"center",justifyContent:"space-between",height:72,position:"sticky",top:0,left:0,right:0,width:"100%",zIndex:50,transition:"all 0.2s",boxSizing:"border-box"}}>
    <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
      ...
      <span style={{fontSize:13,fontWeight:600,color:dark?"#F1F5F9":"#FFFFFF"}}>PadelOS <span style={{fontSize:10,fontWeight:400,opacity:0.65}}>v{APP_VERSION}</span></span>
    </div>
  ...
  async function handleShareBefore(){
    ...
    try{
      ...
      if(isCI&&plan&&plan.rounds&&plan.rounds.length>0){
        try{
          cards.push(buildBreaksRound1Card(effEv,venue,plan,tc,comm.name));
        }catch(breakErr){
          console.error("Break card build failed:", breakErr);
          onToast&&onToast("Couldn't build the break/round-1 card — sharing event info only","err");
        }
      }
