...
// ── Firebase (Phase 1: authentication only — data still lives in localStorage) ──
...
export default function PadelOS() {
  ...
  const linkedMe = users.find(u => u.firebaseUid === authUser?.uid) || null;
  ...
  const requestClaim = (userId) => {
    if (userId === 1) { // the seed owner profile — approving anyone else requires *someone* already linked, so this one is instant
      setUsers(us => us.map(u => u.id===1 ? {...u, firebaseUid:authUser.uid, email:authUser.email||u.email} : u));
      return;
    }
    const id = _crid++;
  ...
  const approveClaim = (reqId) => {
    ...
    if (req) setUsers(us => us.map(u => u.id===req.userId ? {...u, firebaseUid:req.firebaseUid, email:req.email||u.email} : u));
  };
  ...
  const createFreshProfile = () => {
    ...
    setUsers(us => [...us, {id:newId, firebaseUid:authUser.uid, email:authUser.email, nickname:displayName, name:displayName, avatar:ini2(displayName), usr:50, joined:today, isGuest:false}]);
  };
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (d.claimRequests) _crid = Math.max(_crid, ...d.claimRequests.map(r=>r.id), 0) + 1;
      }
  ...
  useEffect(() => {
    try {
      localStorage.setItem('padelos_v10', JSON.stringify({comms, users, venues, notifications, claimRequests, dark}));
    } catch(e) { console.log('Save error', e); }
  }, [comms, users, venues, notifications, claimRequests, dark]);
  ...
  if (!linkedMe) {
    ...
    const unclaimed = users.filter(u => !u.firebaseUid && !pendingUserIds.has(u.id));
  ...
  return <div style={{background:TH?.nav||"#0E1117",borderBottom:`0.5px solid ${TH?.border||"var(--po-bdr)"}`,padding:"0 8px",display:"flex",alignItems:"center",justifyContent:"space-between",height:60,position:"sticky",top:0,left:0,right:0,width:"100%",zIndex:50,transition:"all 0.2s",boxSizing:"border-box",gap:4}}>
    ...
    <div style={{position:"relative",flexShrink:0}} onClick={e=>e.stopPropagation()}>
      ...
      {menu&&<div style={{position:"absolute",right:0,top:42,background:"var(--po-card)",border:"0.5px solid var(--po-bdr)",borderRadius:10,padding:6,minWidth:190,zIndex:100,boxShadow:"0 8px 32px #00000066"}}>
        ...
        {[{i:"👤",l:"My Profile",fn:onProfile},{i:"🛡",l:"Platform Admin",fn:onPlatformAdmin},{i:"🏟",l:"Venues",fn:onVenues},{i:"⚙️",l:"Settings",fn:onSettings},{i:"🚪",l:"Sign Out",fn:()=>{setMenu(false);onSignOut&&onSignOut();},d:true}].map(x=><button key={x.l} onClick={x.fn} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"10px 10px",minHeight:40,borderRadius:7,border:"none",background:"transparent",color:x.d?"#EF4444":"var(--po-sub)",fontSize:13,cursor:"pointer",textAlign:"left"}}>{x.i} {x.l}</button>)}
      </div>}
    </div>
  </div>;
}
