...
function maxPossibleCI(plan, userId){
  // Max theoretical score = winning every round on Court 1 — EXCEPT rounds a player is
  // scheduled to be on break for, which count at break points instead. Without a userId,
  // this returns the flat best-case ceiling (zero breaks) as a rough headline figure only.
  if(!plan)return 0;
  const tc=plan.courts;
  const generatedRounds=plan.rounds.length;
  if(userId==null){
    return generatedRounds*courtPts(1,tc);
  }
  let max=0;
  for(let r=0;r<generatedRounds;r++){
    const onBreak=(plan.breakPlan[r]||[]).includes(userId);
    max += onBreak ? breakPts(tc) : courtPts(1,tc);
  }
  return max;
}
...
const INIT_VENUES = [
  {id:1, name:"Galleria Moon Valley", gov:"القاهرة", area:"القاهرة الجديدة", courts:[{name:"Court 1"},{name:"Court 2"},{name:"Court 3"}], pricePerHour:500, extraFee:30, mapsUrl:"", lat:30.0296, lng:31.4907, status:"approved"},
];
...
function buildStandingsCard(ev,venue,ciStands,tc,plan,communityName){
  ...
  const {c, ctx} = makeCard(w, h);
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  const bestCase = maxPossibleCI(plan);
  let y = drawHeader(ctx, w, "Final standings", `Best case: ${bestCase||"—"} pts (no breaks)`, communityName);
  y += 16;
  y = drawEventStrip(ctx, w, y, ev, venue);

  ciStands.forEach((s,i)=>{
    const maxPts = maxPossibleCI(plan, s.user.id);
...
function buildResultsTableCard(ev,venue,plan,ciStands,tc,communityName){
  ...
  ctx.fillStyle = COLORS.bg; ctx.fillRect(0,0,w,h);
  const bestCase = maxPossibleCI(plan);
  let y = drawHeader(ctx, w, "Match results", `${plan.rounds.length} rounds · Best case ${bestCase||"—"} pts`, communityName);
  y += 16;
  y = drawEventStrip(ctx, w, y, ev, venue);
  ...
  useEffect(() => {
    ...
    const updatedUsers = users.map(u => {
      ...
      completedCI.forEach(ev => {
        ...
        const plan = ev.plan;
        const maxPts = maxPossibleCI(plan, u.id);
        if (maxPts <= 0) return;
  ...
  const stats = members.map(u=>{
    ...
    completedEvents.forEach(ev=>{
      ...
      if(ev.type==="closed_ind"&&ev.plan){
        ...
        const s=stands.find(s=>s.user.id===u.id);
        if(s){wins+=s.wins; totalPts+=s.pts; totalMaxPts+=maxPossibleCI(ev.plan,u.id);}
      }
      if(ev.type==="closed_teams"&&ev.plan){
    ...
    <table style={{borderCollapse:"separate",borderSpacing:0,width:"100%",minWidth:plan.rounds.length*88+260}}>
      ...
      <tbody>
        {ciStands.map((s,i)=>{
          ...
          const mp = maxPossibleCI(plan, s.user.id);
...
function EvDetail({ev,comm,users,venues,me,onBack,onEditEvent,onRegister,onCheckIn,onAddMember,onAddGuest,onVote,onResolveType,onCloseEvent,onStartCI,onSetWinCI,onNextRound,onSwap,onEditBreak,onRegenerateBreaks,onStartCT,onSetWinCT,onApplyPromo,onNextCTLadder,onSwapCTLadder,onRemoveFromEvent,onEditGuestUsr,onEditEventUsr,onToast,onDuplicate,onDelete,onArchive,onUnarchive,onViewProfile,onSwapCTBreak,onRegenCTBreaks,onToggleExempt,onTogglePaid,onUpdateEventFinance,onSetMatchModeStart}){
  ...
  function PChip({p,ri}){
    ...
    if(isTgt&&plan){
      ...
      histBadge = wasLastPartner?{label:"🔗 last partner",color:"#EF4444"}:cnt>0?{label:`×${cnt} played`,color:"#F59E0B"}:{label:"✨ new",color:"#34D399"};
    }
  ...
  return <>
    ...
    {tab==="standings"&&isCI&&<>
      ...
      {plan&&<div style={{marginBottom:10,padding:"8px 12px",background:"#6366F122",border:"0.5px solid #6366F144",borderRadius:8,fontSize:12,color:"#A5B4FC",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span>🎯 Best case (no breaks): <b>{maxPossibleCI(plan)} pts</b> ({plan.rounds.length} rounds × {courtPts(1,tc)}pts) — players with a scheduled break have a lower personal max, see PES% below</span>
      </div>}
      {ciStands.length===0?<Card><div style={{textAlign:"center",color:"var(--po-dim)",fontSize:13,padding:"24px 0"}}>Record winners to see standings.</div></Card>:<>
        {ciStands.map((s,i)=>{const mp=maxPossibleCI(plan,s.user.id),pes=mp>0?Math.round((s.pts/mp)*100*10)/10:0;return <Card key={s.user.id}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,background:i<3?"#6366F133":"var(--po-bdr)",color:i===0?"#FBBF24":i===1?"#94A3B8":i===2?"#CD7C2F":"var(--po-dim)"}}>{i+1}</div><Av u={s.user} size={34}/><div style={{flex:1}}><div style={{fontWeight:600,fontSize:14,color:"var(--po-text)"}}>{s.user.nickname}</div><div style={{fontSize:11,color:"var(--po-dim)"}}>{s.wins} wins · {s.breaks} breaks · {s.played} played</div></div><div style={{textAlign:"right",marginRight:8}}><div style={{fontSize:14,fontWeight:700,color:"#A5B4FC"}}>{pes}%</div><div style={{fontSize:9,color:"var(--po-dim)"}}>PES</div></div><div style={{textAlign:"right"}}><div style={{fontSize:22,fontWeight:700,color:"#6366F1"}}>{s.pts}</div><div style={{fontSize:10,color:"var(--po-dim)"}}>pts</div></div></div></Card>;})}
    ...
    {tab==="standings"&&isCT&&<>
      ...
      {ctStands.length===0
        ...
        : <>
          {/* Typical max score for Ladder — a rounded approximation; each team's row below shows their own exact max */}
          {plan?.format==="ladder"&&plan?.rounds?.length>0&&(()=>{
            const maxPts = ctEventMaxPts(plan);
            return <div style={{fontSize:11,color:"var(--po-dim)",marginBottom:8,textAlign:"right"}}>Typical max (~avg breaks): <b style={{color:"var(--po-text)"}}>{maxPts} pts</b> — exact per team below</div>;
          })()}
